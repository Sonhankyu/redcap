...
elseif (isset($_GET['__dashboard']) && !isset($_GET['a']) && !isset($_GET['sq']) && !isset($_GET['s']) && !isset($_GET['hash']))
{
    require_once dirname(dirname(__FILE__)) . "/Config/init_global.php";
	// If public dashboards are disabled, then stop here with an error message
    if ($GLOBALS['project_dashboard_allow_public'] == '0') {
		Survey::exitSurvey($lang['global_207']);
    }
	// Get PID from hash
	...
	if (!is_numeric($_GET['pid'])) {
		Survey::exitSurvey($lang['dash_36']);
    }
	require_once dirname(dirname(__FILE__)) . "/Config/init_project.php";
...
elseif (isset($_GET['__report']) && !empty($_GET['__report']))
{
    // Call init_global
    require_once dirname(dirname(__FILE__)) . "/Config/init_global.php";
    // If public reports are disabled, then stop here with an error message
    if ($GLOBALS['reports_allow_public'] == '0') {
        Survey::exitSurvey($lang['report_builder_189']);
    }
    // Get PID from hash
    list ($pid, $report_id, $report_title) = DataExport::getReportInfoFromPublicHash($_GET['__report']);
    $_GET['pid'] = $pid;
    if (!is_numeric($_GET['pid'])) {
        Survey::exitSurvey($lang['report_builder_178']);
    }
    // File download
...
if (!$surveys_enabled || $survey_enabled < 1 || $date_deleted != '' || $status == 2 || $completed_time != '') {
	...
	$offlineText = $useCustomText ? $offline_instructions : $lang['survey_219'];
...
$group_id = (empty($Proj->groups)) ? null : Records::getRecordGroupId(PROJECT_ID, $fetched);
if (!is_numeric($group_id)) $group_id = null;
Hooks::call('redcap_survey_page_top', array(PROJECT_ID, (is_numeric(isset($_POST['__response_id__']) ? $_POST['__response_id__'] : '') ? (isset($fetched) ? $fetched : null) : null), $_GET['page'], $_GET['event_id'], $group_id, $_GET['s'], (isset($_POST['__response_id__']) ? $_POST['__response_id__'] : ''), $_GET['instance']));
...
var stopAction1 = '<?php echo js_escape($lang['survey_1311']) ?>';
var stopAction2 = '<?php echo js_escape($lang['survey_1312']) ?>';
...
if ($isPromisInstrument) {
	...
	print RCView::div(array('id'=>'stopActionPrompt','title'=>$lang['survey_01']), RCView::h6(array('class'=>'boldish'), $lang['survey_02']) . $lang['survey_1313']);
