...
<form id="survey_settings" action="<?php echo $_SERVER['REQUEST_URI'] . ((isset($_GET['redirectInvite']) && $_GET['redirectInvite']) ? "&redirectInvite=1" : "") ?>" method="post" enctype="multipart/form-data">
	<table cellspacing="3" style="width:100%;">
		...
		<tr>
			<td colspan="3">
				<div id="survey_enabled_div" class="clearfix <?php echo($survey_enabled ? 'darkgreen' : 'red') ?>" style="max-width:910px;margin: -5px -7px 0px;font-size:12px;">
					...
					<div style="float:left;">
						...
						<select name="survey_enabled" class="x-form-text x-form-field" style="margin-bottom:3px;"
							onchange="if ($(this).val()=='1'){
							    $('#survey_enabled_img').attr('src',app_path_images+'accept.png');$('#survey_enabled_div').removeClass('red').addClass('darkgreen');
							} else {
							    $('#survey_enabled_img').attr('src',app_path_images+'delete.png');$('#survey_enabled_div').removeClass('darkgreen').addClass('red');
							}">
						...
						<span class="newdbsub ml-4"><?php echo $lang['survey_377'] ?></span>

                        <div class="fs13" style="margin:10px 0 5px 25px;width:550px;">
                            <?php echo $lang['design_945'] ?><button id="offline_instructions_btn" class="btn-defaultrc btn btn-xs ml-2 <?php if (hasPrintableText($offline_instructions)) print "hide"; ?>" onclick="$(this).hide();$('#offline_instructions_div').show();return false;"><?php echo $lang['survey_1331'] ?></button>
                        </div>
                        <div id="offline_instructions_div" style="margin:10px 0 0 25px;width:550px;<?php print (hasPrintableText($offline_instructions) ? "" : "display:none;") ?>">
                            <!-- Offline instructions -->
                            <textarea style="width:98%;height:220px;" name="offline_instructions" class="mceEditor"><?php echo htmlspecialchars(label_decode($offline_instructions), ENT_QUOTES) ?></textarea>
                            <!-- Piping link -->
		...
		<tr>
			<td valign="top" style="width:20px;padding-top:3px;">
                <i class="fas fa-file-pdf fs15" style="color: #D00000;"></i>
            </td>
			<td valign="top" style="width:290px;font-weight:bold;padding-bottom:15px;">
        ...
        <tr>
            <td valign="top" style="width:20px;padding-top:8px;">
                <i class="fas fa-file-pdf fs15" style="color: #D00000;"></i>
            </td>
            ...
            <td valign="top" style="padding-left:15px;padding-bottom:20px;padding-top:5px;">
				...
				print RCView::select(array('name'=>'pdf_save_to_event_id', 'class'=>'x-form-text x-form-field mr-1', 'style'=>'max-width:150px;'.($longitudinal ? "" : "display:none;")),
				...
				?>
                <div class="cc_info" style="margin-top: 10px;"><?php echo $lang['survey_1305']; ?></div>
                <?php if ($Proj->hasRepeatingFormsEvents()) { ?><div class="cc_info" style="margin-top: 10px;"><?php echo $lang['survey_1307']; ?></div><?php } ?>
            </td>
        </tr>
		...
		<tr>
			<td valign="top" style="width:20px;">
                <input type="checkbox" style="position:relative;top:2px;" id="end_survey_redirect_next_survey" name="end_survey_redirect_next_survey" <?php echo (isset($end_survey_redirect_next_survey) && $end_survey_redirect_next_survey == '1' ? 'checked' : '') ?>>
            </td>
			<td valign="top" style="width:200px;padding-bottom:3px;" colspan=2>
				...
				if (Survey::surveyQueueEnabled(PROJECT_ID)) {
					print RCView::div(array('class'=>'text-danger fs11 my-1 ml-2'), '<i class="fas fa-exclamation-circle"></i> '.$lang['survey_1299']);
				}
				?>
            </td>
		</tr>
        ...
        <!-- Stop Actions -->
        <tr>
            <td colspan="3" style="padding-bottom:20px;font-weight:bold;font-size:14px;color:#800000;">
