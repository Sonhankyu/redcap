...
if (isset($_GET['export']) && !($_GET['export'] == 'all' || $_GET['export'] == 'odm_project' || $_GET['export'] == 'pdf' || $_GET['export'] == 'odm' || $_GET['export'] == 'r' || $_GET['export'] == 'spss' || $_GET['export'] == 'stata' || $_GET['export'] == 'excel' || $_GET['export'] == 'sas')) {
	unset($_GET['export']);
}
...
if (isset($_GET['id']))
{
	...
	if ($_GET['id'] == "")
	{
		print  "<p style='padding-bottom:10px;'>
                {$lang['docs_33']} \"$upload_button_text\" {$lang['docs_34']}
                <span style=\"color:#555;font-size:13px;\">(".$lang["data_entry_63"] . " " . maxUploadSizeFileRespository() . "MB)</span>
                </p>";
	}
	...
	elseif (isset($_GET['type']) && $_GET['type'] == 'export' && $user_rights['data_export_tool'] > 0)
	{
		...
		if (isset($_GET['export'])) {
			switch ($_GET['export']) {
				...
				case "pdf": $limit_export = "AND right(docs_name,4) = '.pdf'"; break;
				default:
					// Show all
			}
		}
		...
		if (!($qrs_pubs_num_rows < 1 && (!isset($_GET['export']) || $_GET['export'] == ''))) {
			...
			$page_instructions .=  "<option value='pdf'"; if (isset($_GET['export']) && $_GET['export'] == 'pdf') $page_instructions .= " selected";
			$page_instructions .=  ">".$lang['docs_73']."</option>";
			...
			$filteringByStatsPackage = (isset($_GET['export']) && ($_GET['export'] == 'r' || $_GET['export'] == 'sas' || $_GET['export'] == 'spss' || $_GET['export'] == 'stata'));
			$export_rows = array();
			$docs_ids = array();
			while ($rs_pubs = db_fetch_assoc($qrs_pubs))
			{
				// Set docs_type
                if (strtolower(substr($rs_pubs['docs_name'], -4)) == '.pdf') {
					$rs_pubs['docs_type'] = $docs_type = 'PDF';
                } else {
				...
				// Add doc_id to array
				if ($rs_pubs['docs_type'] != 'PDF' && !($filteringByStatsPackage && $rs_pubs['docs_type'] == 'DATA')) {
					$docs_ids[] = $rs_pubs['docs_id'];
				}
			}

			// Get export details of each data export
            $exportDetails = [];
            if (!empty($docs_ids)) {
				// Display link to logging details
				$sql = "select d.docs_id, l.data_values
                        ...
                        where d.project_id = p.project_id and d.export_file = 1 and d.docs_id = t.docs_id and t.doc_id = e.doc_id
                        and timestamp(l.ts) = e.stored_date and l.project_id = d.project_id and l.event = 'DATA_EXPORT'
                        and d.docs_id in (".prep_implode($docs_ids).") and d.project_id = $project_id";
				$q = db_query($sql);
				while ($row = db_fetch_assoc($q)) {
					...
					$exportDetails[$row['docs_id']] = $jsonStr;
				}
            }
				...
				foreach ($export_group as $export_group_key=>$rs_pubs)
				{
					...
					switch ($docs_type)
					{
						...
						case "R":
							...
							break;
						case "PDF":
							$docs_header = $lang['data_export_tool_171'];
							$docs_logo = "pdf_icon_large.gif";
							$instr = "";
							break;
						case "DATA":
					...
					if (isset($exportDetails[$rs_pubs['docs_id']])) {
						$page_instructions .= "<div class='mt-2 wrap fs11 text-secondary' style='line-height: 1.2;'>"
                                            . RCView::b($lang['data_export_tool_250'])."<br>".implode(", ", $exportDetails[$rs_pubs['docs_id']])."</div>";
                    }
					...
					if ($docs_type == "PDF") {
						...
						$page_instructions .=  "<div style='text-align:left;padding:5px 0 1px;$sendItLinkDisplay'>
													...
													<div id='sendit_$docs_id' style='display:none;padding:4px 0 4px 6px;'>
														<div>
															&bull; <a href='javascript:;' onclick='popupSendIt($docs_id,2);' style='font-size:10px;'>{$lang['docs_54']}</a>
														</div>
													</div>
												</div>
												</td>
												</tr>";
					} elseif ($docs_type == "DATA") {
						//EXCEL
						if ($isLabelsFile) {
							// Label file only
