...
if (!defined("FORCE_INIT_GLOBAL"))
{
	...
	elseif (isset($_GET['s']) && !empty($_GET['s']))
	{
		...
		$sql = "select r.completion_time from redcap_surveys_participants p, redcap_surveys_response r 
				where p.participant_id = r.participant_id and p.survey_id = '".db_escape($Proj->forms[$_GET['page']]['survey_id'])."' and p.event_id = '".db_escape($_GET['event_id'])."'
				and r.record = '".db_escape($_GET['record'])."' and r.instance = '".db_escape($_GET['instance'])."'
