...
class DataAccessGroupsController extends Controller
{
    ...
    {
        Logging::logEvent("", "redcap_data_access_groups", "MANAGE", PROJECT_ID, "project_id = " . PROJECT_ID, "Download DAGs");

        $dags = Project::getDAGRecords();
        $content = (!empty($dags)) ? arrayToCsv($dags) : 'data_access_group_name,unique_group_name';
    ...
    {
        Logging::logEvent("", "redcap_data_access_groups_users", "MANAGE", PROJECT_ID, "project_id = " . PROJECT_ID, "Download User-DAG Assignment");
    ...
    public function uploadDag()
    {
        ...
        if ($csv_content != "")
        {
            ...
            if ($commit && empty($errors)) {
                ...
                Logging::logEvent("", "redcap_data_access_groups", "MANAGE", PROJECT_ID, "project_id = " . PROJECT_ID, "Upload DAGs");
            } else {
    ...
    public function uploadUserDag()
    {
        ...
        if ($csv_content != "")
        {
            ...
            if ($commit && empty($errors)) {
                ...
                db_query("COMMIT");
                db_query("SET AUTOCOMMIT=1");
                Logging::logEvent("", "redcap_user_rights", "MANAGE", PROJECT_ID, "project_id = " . PROJECT_ID, "Upload User-DAG Assignment");
            } else {
                // ERROR: Roll back all changes made and return the error message
                db_query("ROLLBACK");
                db_query("SET AUTOCOMMIT=1");
