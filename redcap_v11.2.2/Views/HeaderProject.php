...
{
	$appsMenu .= "<div class='hang'><i class=\"fas fa-cube fs14\" style='position:relative;top:1px;margin-right:1px;margin-left:1px;'></i>&nbsp;<a href=\"" . APP_URL_EXTMOD_RELATIVE . "manager/project.php?pid=$project_id\">{$lang['global_142']}</a></div>";
}
$appsMenu .= "</div>";
...
if($patient = FhirLauncher::getSessionData('patient-data'))
{
	...
	<script type="text/javascript">
	const destroyFhirSession = function() {
		const url = app_path_webroot_full+'ehr.php'
	...
	$(function(){
		const checkIfInIFrame = function() {
			if ( window.location !== window.parent.location ) {
				console.log("The page is in an iframe")
			} else {
				console.log("The page is not in an iframe. destroying FHIR session")
				destroyFhirSession()
			}
		}
		// checkIfInIFrame()
...
<div class="container-fluid mainwindow">
	...
	<div id="mainwindowsub" style="flex-wrap:nowrap;-ms-flex-wrap:nowrap;" class="row row-offcanvas row-offcanvas-left <?php echo ($_SESSION["mc_open"] == '1' && $user_messaging_enabled == 1 ? 'body-override' : '') ?>" data_open="0">
        <!-- left-hand menu -->
		...
		<div id="center" class="col">
            ...
            <div id="subheader" class="d-print-none">
				...
				<?php } ?>
				<?php if (defined("PROJECT_ID")) { ?><div id="subheaderDiv2" class="bot-left"><?php echo decode_filter_tags($app_title) ?><span class="browseProjPid fs11 opacity65 ml-4 d-print-none">PID <?=PROJECT_ID?></span></div><?php } ?>
			</div>
<?php
