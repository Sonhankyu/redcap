...
if (!empty($surveysettings_columns)) {
    ...
    $sqlColumns = "project_id,form_name";
    $sqlValues = "$project_id,'".db_escape($this_form)."'";
    foreach ($validSurveySettingsDirectories as $surveyDirectory) {
        if (isset($surveyEdocs[$surveyDirectory]) && is_numeric($surveyEdocs[$surveyDirectory])) {
            $sqlColumns .= ",$surveyDirectory";
            $sqlValues .= ",".checkNull($surveyEdocs[$surveyDirectory]);
        }
    }
    foreach ($surveysettings_columns as $column => $surveysettings_column) {
        $sqlColumns .= ",$surveysettings_column";
        $sqlValues .= ",".checkNull($surveysettings_values[$column]);
    }
    // Save survey info
    $sql = "replace into redcap_surveys (".$sqlColumns.") values (".$sqlValues.");";
