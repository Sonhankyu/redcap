	...
	foreach ($metadata as $row)
	{
		...
		if (strpos($row['misc'], '@') !== false) {
			...
			if (isset($this_misc_match[1]) && !empty($this_misc_match[1])) {
				...
				$string_data1 .= " 'action_tag_class_design'=>'".js_escape(str_replace(array("\'","="), array("'",""), implode(" ", $tagNames)))."',";
			}
		}
		...
		if ($field_name != $Proj->table_pk) {
			$string_data1 .= ", 'field_embed'=>'<div var=\"$field_name\" class=\"rc-field-embed-designer $fieldIsEmbeddedOnInstrument1 float-right mr-1 bg-success text-white\" style=\"padding:1px 3px;\"> <i class=\"fas fa-info-circle\"></i> ".htmlspecialchars($lang['design_793'], ENT_QUOTES)."</div>'";
		}
		// END
		$string_data1 .= " );";
