...
if ($_POST['action'] == 'search')
{
    ...
    if ($keyword == '') { // Display message if user neither selected classification nor entered any keyword
        ...
        $searchResults = $apiObj->search($keyword, $searchOrg, $page_num);
    ...
    if (!empty($searchResults)) {
        ...
        if ($keyword == '') { // Display message if user neither selected classification nor entered any keyword
            ...
            if (!empty($resultRows) && empty($resultRows['message'])) {
                ...
                $helpText = '<a href="javascript:;" class="help ml-1" title="'.js_escape2($lang['design_923']).'" onclick="simpleDialog(\'' . $lang['design_922'] . '\',\'' . $lang['design_923'] . '\')">?</a>';
                ...
                foreach ($resultRows as $row) {
                    ...
                    if (isset($row['questionTexts']) && count($row['questionTexts']) > 0) {
                        ...
                        $questionTexts = '<div style="padding-top: 30px;">
                                            ...
                                            <select name="select_field_label" class="select-question fs11 ml-3" style="max-width: 400px;">
                    ...
                    switch ($datatype) {
                        ...
                        case 'time':
                            $result_html .= '<input class="x-form-text x-form-field time2 hasDatepicker" style="max-width:70px;" type="text" name="" value="" fv="time" style="font-weight: normal; background-color: rgb(255, 255, 255);">
                                             ...
                                             <button type="button" class="jqbuttonsm ml-2 ui-button ui-corner-all ui-widget">' . $lang['form_renderer_29'] . '</button><span class="df">H:M</span>';
                            break;
                        case 'number':
                            ...
                            break;
                        case 'file':
                            $result_html .= '<a href="javascript:;" class="fileuploadlink"><i class="fas fa-upload mr-1 fs12"></i>' . $lang['form_renderer_23'] . '</a>';
                            break;
                        case 'date':
