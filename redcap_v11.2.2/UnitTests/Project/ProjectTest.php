...
class ProjectTest extends TestCase
{
	...
	public static function update($project_id, $field, $value)
	{
		$sql = "
	...
	public static function create()
	{
		...
		$sql = "
			INSERT INTO redcap_projects (
				project_name, app_title, status
			) VALUES (
				'test_$s', 'Test $s', 0
			)
		";
		$q = db_query($sql);
		$project_id = ($q && $q !== false) ? db_insert_id() : 0;

		if($project_id)
		{
			MetaDataTest::create($project_id);
			MetaDataTest::create($project_id, 'my_first_instrument_complete', 2);
	...
	public function testNumEventsFormDesignated()
	{
		...
		ProjectTest::update($project_id, 'repeatforms', 1);
	...
	public function testGetProjValidateEventIdSurveyId()
	{
		...
		ProjectTest::update($project_id, 'surveys_enabled', 1);
		ProjectTest::update($project_id, 'repeatforms', 1);
	...
	public function testGetProjWithSurveysEnabledDraftMode()
	{
		...
		ProjectTest::update($project_id, 'surveys_enabled', 1);
		ProjectTest::update($project_id, 'status', 1);
		ProjectTest::update($project_id, 'draft_mode', 1);
	...
	*/
	...
	{
		$count = rowCount('redcap_projects');

		$project_id = ProjectTest::create();

		$this->assertGreaterThan(0, $project_id);
		$this->assertGreaterThan($count, rowCount('redcap_projects'));
	}
}
