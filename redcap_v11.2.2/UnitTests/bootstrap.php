...
$GLOBALS['data_file'] = APP_PATH_DOCROOT.'Resources/sql/install_data.sql';

function resetDB()
{
	dropTables(getTables());
	buildTables();
	insertData();
}
...
function insertData()
{
	...
	echo `mysql -h$hostname -u$username -p$password $db < $GLOBALS[data_file]`;
}

function buildTables()
{
	require $GLOBALS['db_file'];
	echo `mysql -h$hostname -u$username -p$password $db < $GLOBALS[sql_file]`;
}

function dropTables($tables=array())
{
	require $GLOBALS['db_file'];
...
function clearTables($tables=array())
{
	require $GLOBALS['db_file'];
	$c = mysqli_connect($hostname, $username, $password, $db);
