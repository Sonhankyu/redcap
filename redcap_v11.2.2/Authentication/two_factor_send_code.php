...
elseif ($two_factor_auth_twilio_enabled && $_POST['type'] == 'voice' && $user_info['user_phone'] != '') {
	...
	try {
		...
		// Make call
		$call = $twilioClient->account->calls->create(TwilioRC::formatNumber($two_factor_auth_twilio_from_number_voice_alt == '' ? $two_factor_auth_twilio_from_number : $two_factor_auth_twilio_from_number_voice_alt), TwilioRC::formatNumber($number_to_call), $question_url, $call_params);
		// Add to table in case they reply to SMS rather than entering the code
