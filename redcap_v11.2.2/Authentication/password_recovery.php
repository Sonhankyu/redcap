...
elseif (isset($_POST['username']))
{
	...
	// Provide a deterministic (but fake) security question where the user does not exist
	if (!$isUser) {
		$lowerUser = strtolower($_POST['username']);
		$sumChars = 0;
		for ($pos=0; $pos < strlen($lowerUser); $pos ++) {
			$sumChars += ord(substr($lowerUser, $pos));
		}
		// Set fake values for this non-existent user
		$securityQid = ($sumChars % 21) + 1;
		$isUser = $isTableBasedUser = true;
	}
