...
abstract class AbstractPseudoQuery{
	...
	function getActualSQL($sql){
		...
		if($parsed['SELECT'] === null || !isset($parsed['SELECT'][0]['expr_type'])){
		...
		if($this->isDataQuery()){
			...
			$defaultOrderBySql = "order by redcap_data.record + 0, redcap_data.record";
			
			$repeatingForms = $this->getProject()->getRepeatingForms();
			if(
				!empty($repeatingForms)
				&&
	...
	function modifyParsedSQL(&$parsed, $clause, $action){
		...
		if($clause === 'SELECT'){
			...
			$clauseSQL = $creator->create(['SELECT' => $dummyParsed['SELECT'], $clause => $parsed[$clause]]);
			...
			if (is_array($subtree)) {
				...
				if($item['base_expr'] === '*'){
					...
					if ($field === 'username') {
						...
						$forTopLevelSelect = $addAs && @$item['alias'] === false;
	...
	private function convertDateDiff(&$item){
		...
		$signed = $getString($oldSubTree[3]) === 'true';
		$unit = $map[$type]['unit'];
