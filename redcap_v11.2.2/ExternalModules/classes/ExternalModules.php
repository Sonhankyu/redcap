...
class ExternalModules
{
	...
	public static function getTestModuleDirectoryPath($prefix = self::TEST_MODULE_PREFIX) {
		return __DIR__ . '/../tests/' . $prefix . '_' . self::TEST_MODULE_VERSION;
	}
	...
	static function initialize()
	{
		...
		if(self::isManagerPage()){
			...
			if(self::isManagerPage('ajax/log.php')){
				$moduleInstance = self::getModuleInstance($_GET['prefix']);
				$frameworkVersion = self::getFrameworkVersion($moduleInstance);
			}

			self::checkCSRFToken($frameworkVersion);
		}

		self::initCSRFDoubleSubmitCookie();
	...
	public static function initCSRFDoubleSubmitCookie(){
		// Set up the token for the next request (or requests if using ajax).
		$newToken = bin2hex(random_bytes(40));
		setcookie('redcap_external_module_csrf_token', $newToken, 0, '/');
	
		// Make the new token available to $module->getCSRFToken() calls
		$_COOKIE['redcap_external_module_csrf_token'] = $newToken;
	}
	...
	public static function query($sql, $parameters = null, $retriesLeft = 2)
	{
		...
		try{
			...
			if($errorCode === 2006 && !self::$shuttingDown){
				...
				self::exitWithError("A 'MySQL server has gone away' error was detected.  It is possible that there was an actual database issue, but it is more likely that REDCap detected this request as a duplicate and killed it.");
			}
			...
			if(empty($dbError) && $e->getMessage() === ExternalModules::tt('em_errors_108')){
				 ...
				 */
				self::exitWithError("A query failed with an empty error.  Please report this error to datacore@vumc.org with instructions on how to reproduce it if possible.");
			}
	...
	private static function exitWithError($message)
	{
		...
		$commandLine = ExternalModules::isCommandLine();

		if(ExternalModules::isSuperUser() || $commandLine){
			if($commandLine){
				echo "\n";
			}
			else{
	...
	static function callHook($name, $arguments, $prefix = null)
	{
		...
		try {
			...
			$startHook = function($prefix, $version) use ($name, &$arguments, &$resultsByPrefix){
	 ...
	 * It lives here to ensure that unit tests detect any breakages (like changes to intro.md).
	 */
	...
	{
		$minVersion = @$config['compatibility']['redcap-version-min'];
		...
		if(!empty($frameworkVersion)){
			...
			$frameworkMinVersion = $minRedcapVersionsByFrameworkVersion[$frameworkVersion]['standard'];
			
			// Per the unit test, this compare method works as expected for null, empty string, and "TBD".
			if(version_compare($frameworkMinVersion, $minVersion, '>')){
	...
	static function setMissingConfigSections(&$config){
		foreach(['permissions', 'system-settings', 'project-settings', 'no-auth-pages', 'no-csrf-pages'] as $key) {
			if(!isset($config[$key])) {
				$config[$key] = array();
			}
		}
	...
	static function hasModuleConfigurationUserRights($prefix=null)
	{
		...
		return in_array($prefix, $rights[self::getUsername()]['external_module_config']);
	}
	...
	static function checkCSRFToken($frameworkVersion){
		...
		else if($frameworkVersion >= 8){
			$errorLanguageKey = 'em_errors_157';
		}
		else{
		...
		else if(!hash_equals($csrfToken, (string) @$_COOKIE['redcap_external_module_csrf_token'])){
