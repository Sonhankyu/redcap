...
class ExternalModulesTest extends BaseTest
{
	...
	function testIsTimeToRun()
	{
		...
		foreach ($offsets as $displacement => $validationMethod) {
			...
			$_SERVER["REQUEST_TIME_FLOAT"] = $datetime->format("U.u");
	...
	function testCallHook_pdfReturnValueDaisyChaining()
	{
		...
		foreach([TEST_MODULE_PREFIX, TEST_MODULE_TWO_PREFIX] as $prefix){
			$this->setConfig(['permissions' => ['redcap_pdf']], true, $prefix);
	...
	function testEnsureFrameworkVersionsFullyImplemented(){
		foreach($this->callPrivateMethod('getMinREDCapVersionsByFrameworkVersion') as $frameworkVersion => $redcapVersions){
			$this->assertNotEmpty($redcapVersions['standard']);
			$this->assertNotEmpty($redcapVersions['lts']);

			$this->assertFileExists(__DIR__ . "/FrameworkV{$frameworkVersion}Test.php", "A unit test class for framework version $frameworkVersion must be created to ensure that unit tests run for that version.");
			$this->assertFileExists(__DIR__ . "/../docs/framework/v$frameworkVersion.md", "Documentation must be added for framework version $frameworkVersion.");
		}
	}
	...
	private function assertNormalizeModuleZip($beforePaths, $afterPaths){
		$zipPath = tempnam(sys_get_temp_dir(), 'em-test-');
