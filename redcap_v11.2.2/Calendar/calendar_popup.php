...
if (isset($_GET['cal_id']) && is_numeric($_GET['cal_id']) && empty($_POST))
{
	...
	if (db_num_rows($q) > 0) {
		...
		if (isset($row['record']) && $row['record'] != "") 
		{
			...
			// Render form list
            $formlist = renderPanel('Data Entry Forms', "<div style='padding:2px 0;'>$formString</div>");
 ...
 */
	...
	if ($_POST['idnumber'] != "") {
		$group_id = db_result(db_query("select value from redcap_data where project_id = $project_id and record = '".db_escape($_POST['idnumber'])."' and field_name = '__GROUPID__' limit 1"), 0);
