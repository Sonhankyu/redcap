...
$(function(){
    ...
    var msgSuccess = "<img src='"+app_path_images+"tick.png'> <b>SUCCESSFUL!</b> - Communicated successfully with the Field Bank NLM server (https://cde.nlm.nih.gov).";
    var msgError = "<b>FAILED!</b> - Could NOT communicate with the Field Bank NLM server (https://cde.nlm.nih.gov).";
...
//PAGE HEADER
print RCView::h4(array('style'=>'margin-top:0;'), '<i class="fas fa-clipboard-check"></i> ' . $lang['control_center_443'] . " <span class=\"text-secondary ml-2\">(REDCap $redcap_version)</span>");
print  "<p>
...
if (version_compare($db_version, System::getMinMySQLVersion(), '<')) {
	print "<div class='red'><img src='".APP_PATH_IMAGES."exclamation.png'> <b>NOT using MySQL/MariaDB ".System::getMinMySQLVersion()." or higher
			- CRITICAL:</b> It is required that you upgrade your database server to a more recent supported version of MySQL or MariaDB 
			(you are currently running $db_type $db_version). Some important features of REDCap will not function correctly unless you are on the minimum version or higher.</div>";
} else {
	print "<div class='darkgreen'><img src='".APP_PATH_IMAGES."tick.png'> <b style='color:green;'>Using $db_type ".System::getMinMySQLVersion()." or higher</b></div>";
}
...
if ($q && db_num_rows($q) > 0)
{
	...
	if (stripos($row['Value'], 'rowid_filter=on') !== false) {
		print "<div class='red'><img src='".APP_PATH_IMAGES."exclamation.png'> <b>Database configuration issue:</b>
		        There is a configuration issue with the MySQL setting named \"optimizer_switch\" that needs to be fixed. If not fixed, if may cause some things not to function correctly in REDCap
                or might cause the database server to crash unexpectedly.
		        To fix this, add the line<br><code><b>optimizer_switch=rowid_filter=OFF</b></code><br>
		        under the [mysqld] section in your MY.CNF (or MY.INI) configuration file for MySQL/MariaDB. If there is already a line in the file that begins with \"optimizer_switch\",
		        then add this new line directly below it.
		        After that new line has been added, save that configuration file, and then restart the MySQL/MariaDB service. Then reload this page to ensure the fix has successfully taken effect.</div>";
    }
}
...
if ($MySQLTunerRecs == "") {
    // All good
	print "<div class='darkgreen'><img src='".APP_PATH_IMAGES."tick.png'> <b style='color:green;'>".db_get_server_type()." configuration settings appear sufficient for optimal performance and stability</b></div>";
} else {
...
if (System::isWindowsServer() && Cron::checkIfCronsActive()) {
	...
	if ($mysqlUptime > 86400*$daysToCheckCronInstances) {
		...
		if ($cron_status != '1') {
			print "<div class='gray'><span style='color:#A00000;'><i class=\"fas fa-lightbulb\"></i> <b>Possible issue with Windows Scheduled Task for the REDCap Cron Job - SUGGESTION:</b></span>
                It appears that <u>REDCap's cron job has not been running every minute</u> according to the cron job's log over the past $daysToCheckCronInstances days.
                This might imply that the scheduled task on your Windows server might not be configured correctly.
                Check the Windows Task Scheduler on your REDCap web server and make sure the scheduled task for the REDCap Cron job is set 
                to \"<u>Run a new instance in parallel</u>\" under the Settings tab for that task. If not, then change it to that setting.<br><br>
                If you are certain that this is not the case, then you may ignore this suggestion; however,
                there might be other problems that exist if for some reason the REDCap Cron Job has not been running every minute as expected.</div>";
        }
    }
}
...
if (!empty($deleteRedcapDirs)) {
	...
	print "<div class='yellow'><img src='".APP_PATH_IMAGES."exclamation_orange.png'> <b>Remove insecure REDCap versions from web server
				- RECOMMENDED:</b> It is recommended that you remove all the REDCap version directories from your web server that are listed below.
				These versions are known to have security vulnerabilities that could possibly be exploited by malicious attackers if left on your REDCap web server.
...
if ($field_bank_enabled) {
	print 	RCView::div(array('class'=>'gray'),
                RCView::b("External Service Check: Checking communication with Field Bank NLM services") . RCView::br() .
