	...
	<tr>
		...
		<td>
			<input type='text' class='x-form-text x-form-field' id='username' name='username' maxlength='255' <?php echo (empty($user_obj->ui_id) ? '' : 'style="background-color:#ddd;"') ?>
				onblur="if (this.value.length > 0) {if(!chk_username(this,<?php print ($auth_meth == 'ldap' || $auth_meth == 'aaf' || $auth_meth == 'aaf_table' || $auth_meth == 'ldap_table') ? "true" : "false" ?>)) return alertbad(this,'<?php echo $lang['rights_354'] ?>'); }"
