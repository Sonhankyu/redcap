...
$usernameTextbox = RCView::div(array('class'=>'input-group ml-1 mb-1 fs12 nowrap'),
...
foreach ($admins as $this_ui_id=>$attr)
{
    ...
    if ($attr['username'] == '') {
        ...
        $user_display = RCView::span(array('class'=>'mx-2'.($this_ui_id == UI_ID ? ' text-danger' : '')),
                            RCView::span(array('class'=>'fs13 boldish mr-1'), $attr['username']) .
...
<h4 style="margin-top: 0;"><i class="fas fa-user-shield"></i> <?php echo $lang['control_center_4735'] ?><?php if (!ADMIN_RIGHTS) { print "<span class='browseProjPid fs12 font-weight-normal ml-3'>{$lang['control_center_4751']}</span>"; } ?></h4>
