...
function getRecordsFlat()
{
	...
	return Records::getData($post['projectid'], $post['format'], $records, $fields, $events, $post['dataAccessGroupId'], false,
							...
							false, false, false, false, true, null, 0, false, (isset($post['csvDelimiter']) ? $post['csvDelimiter'] : ","), (isset($post['decimalCharacter']) ? $post['decimalCharacter'] : null));
}
...
function getRecords()
{
	...
	if ($longitudinal) {
		...
		$sql = "SELECT record, event_id, field_name, value, instance
				...
				ORDER BY record regexp '^[A-Z]', abs(record), replace(replace(record,'_',''),'-','')*1, record";
        if ($Proj->hasRepeatingFormsEvents()) {
            $sql .= ", instance";
        }
	}
