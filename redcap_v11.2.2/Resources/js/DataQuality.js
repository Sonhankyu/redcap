...
function enableRuleTableEdit() {
	...
	$('#table-rules .editlogic').click(function(){
		...
		var thisRuleId = $(this).attr('rid');
		$(this).html( '<textarea id="input_rulelogic_id_'+thisRuleId+'" class="x-form-field notesbox" onfocus=\'openLogicEditor($(this))\' style="font-size:12px;height:60px;margin:4px 0;width:95%;resize:auto;" onblur=\'if(!checkLogicErrors(this.value,1)){validate_logic(this.value,"",0);}\'>'+thisRuleLogic+'</textarea>'
...
function executeRulesAjax(rule_ids,show_exclusions,replaceRuleTable,action) {
	...
	var thisAjax = $.post(app_path_webroot+'DataQuality/execute_ajax.php?pid='+pid, { record: $('#dqRuleRecord').val(), rule_ids: rule_ids, show_exclusions: show_exclusions, action: action }, function(data){
		...
		if (replaceRuleTable == 0) {
			...
			if (discrep > 0) {
				discrep_text += "<a href=\"javascript:;\" onclick=\"javascript: exportRulesDiscrepancies('" + rule_id + "');\" style='font-size:11px;text-decoration:underline;'>export</a>";
