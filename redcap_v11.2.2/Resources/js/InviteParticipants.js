...
function deleteMultipleInvites() {
	...
	if (invitesChecked == 0) {
		...
		$('#prevent_retrigger_multi').prop('checked', true);
	}
}
...
function deleteSurveyInvite(email_recip_id, reminder_num) {
	$.post(app_path_webroot+'Surveys/survey_invitation_ajax.php?pid='+pid,{ email_recip_id: email_recip_id, reminder_num: reminder_num, action: 'view_delete' }, function(data){
		...
		var json_data = jQuery.parseJSON(data);
		// Display dialog
		simpleDialog(json_data.content,json_data.title,null,600,null,'Cancel','deleteSurveyInviteDo('+email_recip_id+','+reminder_num+')','Delete invitation');
		$('#prevent_retrigger_single').prop('checked', true);
	});
}
