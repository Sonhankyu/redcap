...
class API {
     ...
     * add a revision on the database
     */
    runRevision(revision_id, mrn, options={}) {
        ...
        post_params.append('revision_id', revision_id)
        post_params.append('mrn', mrn)
        post_params.append('background', background)
        post_params.append('send_feedback', send_feedback)
    ...
    searchMrns({query, start=0, limit=10})
    {
        var params = {
            route: `${this.route}:searchMrns`,
            headers: {
                'Content-Type': 'multipart/form-data'
            },
            query,
