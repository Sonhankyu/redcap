...
function openAddUserPopup(username,role_id) {
    ...
    $.post(app_path_webroot+'UserRights/edit_user.php?pid='+pid, { username: username, role_id: role_id }, function(data){
        ...
        if ($('#editUserPopup #dialog_title').length) {
            ...
            // Open dialog
            $('#editUserPopup').dialog({ bgiframe: true, modal: true, width: 900,
                open: function(){
...
function enablePageJS() {
    ...
    $('#addUserBtn').click(function(){
        ...
        if (!chk_username(document.getElementById('new_username'),((auth_meth == 'ldap' || auth_meth == 'ldap_table' || auth_meth == 'aaf' || auth_meth == 'aaf_table') ? 1 : 0))) {
            simpleDialog(lang.rights_354+((auth_meth == 'ldap' || auth_meth == 'ldap_table' || auth_meth == 'aaf' || auth_meth == 'aaf_table') ? ' '+lang.rights_401 : ''),null,null,null,"$('#new_username').trigger('focus')");
            return;
        }
        // Open dialog to add new user with custom rights
    ...
    $('#assignDagRoleBtn').click(function(){
        ...
        if ($('#userClickTooltip').css('display') == 'none') {
            ...
            if (!chk_username(document.getElementById('new_username_assign'),((auth_meth == 'ldap' || auth_meth == 'ldap_table' || auth_meth == 'aaf' || auth_meth == 'aaf_table') ? 1 : 0))) {
                simpleDialog(lang.rights_354+((auth_meth == 'ldap' || auth_meth == 'ldap_table' || auth_meth == 'aaf' || auth_meth == 'aaf_table') ? ' '+lang.rights_401 : ''),null,null,null,"$('#new_username_assign').trigger('focus')");
                return;
            }
        } else {
...
function setUserDagRoleSelected(username) {
    if (username != '' && username != lang.rights_160) {
