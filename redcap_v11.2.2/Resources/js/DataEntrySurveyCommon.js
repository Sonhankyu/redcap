    ...
    $('#questiontable tr.\\@SETVALUE, #questiontable tr.\\@PREFILL, #questiontable tr.\\@DEFAULT, #questiontable tr.\\@NOW, #questiontable tr.\\@TODAY, #questiontable tr.\\@NOW-SERVER, #questiontable tr.\\@TODAY-SERVER, #questiontable tr.\\@NOW-UTC, #questiontable tr.\\@TODAY-UTC').each(function(i, row) {
        ...
        if (field.val() != '') { // Only do anything if the field has a value
            ...
            if ($(field).is('select')) {
                ...
                // Set @PREFILL/@SETVALUE/@DEFAULT field to show that it changed value
                if (($('#'+fname+'-tr').hasClass('@DEFAULT') || $('#'+fname+'-tr').hasClass('@PREFILL') || $('#'+fname+'-tr').hasClass('@SETVALUE')) && $(this).attr('ignoreDefault') != '1') {
...
function doFieldEmbedding()
{
    ...
    $('#questiontable .rc-field-embed').each(function ()
    {
        ...
        if ($(':input', source_tr).length == 0) {
            ...
            if (lastTdDesc.children().length == 0) {
            ...
            source_data = $("<div>").append(header_table).append(data_table).append(source_tr.find('.resetLinkParent').clone());
        } else if (align === "right") {
       ...
       if (source_tr.hasClass('@INLINE')) $(this).addClass('file-upload-inline-embed');
        ...
        if (isSurveyPage === false && displayIcons === true) {
            ...
            if (typeof source_tr.attr('mtxgrp') == 'undefined') { // Icons cannot yet be displayed for embedded matrix fields!
                ...
                //$(this).append('<span class="rc-field-icons" style="width:'+iconsTd.outerWidth()+'px !important;">'+iconsTd.html()+'</span>');
                // Place the icons into a span tag so you can do CSS to control their wrapping
        ...
        setTimeout(function(){
            if ($('a#'+name+'-link.filedownloadlink').length) {
                ...
                if (in_array(fileext, valid_image_suffixes)) {
                    // image
                    var html = "<img src='"+srcView+"' style='max-width:100%;' alt='"+filename+"'>";
                } else if (fileext == 'pdf') {
                    // pdf
                    var html = "<object data='"+srcView+"' type='application/pdf' style='width:100%;max-width:100%;height:300px;'><iframe src='"+srcView+"' style='width:100%;max-width:100%;height:300px;border:none;'></iframe></object>";
                }
                var link = "<a target='_blank' href='"+srcDownload+"' style='text-decoration:underline;'>"+filename+"</a>";
                if (val == missing_data_replacement_js) {
                    html = link = missing_data_replacement_js;
                }
                $(piping_receiver_class_field_js+event_id+'-'+name+'-inline').html(html);
...
var pageHasEmbeddedFields = (document.querySelectorAll('.rc-field-embed').length > 0);
    ...
    if (logic == false) {
        ...
        if (fldLen > 0) {
            ...
            if (showEraseValuePrompt && !overrideEraseValuePrompt) {
                ...
                if (overrideEraseValuePrompt && !parentField && document.getElementById('icon-showfield-'+this_field) == null) {
                    var a = document.querySelectorAll('#' + this_field + '-tr>td');
                    var lastTd = a[a.length-1];
                    var top = round(lastTd.getBoundingClientRect().top + window.scrollY)+5;
                    var left = round(lastTd.getBoundingClientRect().left)+7;
                    ...
                    icon.setAttribute('style', 'cursor:pointer;cursor:hand;top:'+top+'px;left:'+left+'px;');
                    icon.setAttribute('title', 'Field should be hidden by branching logic but is shown because it has a value.');
                    document.getElementById('center').appendChild(icon);
                }
            }
            if (eraseIt) {
                showHideAction = 'hide';
...
function enhanceChoiceSelect(ob, e, maxchecked) {
    // Ignore if clicked on a text field embedded inside the enhanced choice
    nodeName = '';
    try {
...
function triggerStopAction(ob) {
    ...
    $('#stopActionPrompt').dialog({ bgiframe: true, modal: true, width: (isMobileDevice ? $(window).width() : 600),
        close: function(){
...
function enableDataEntryRowHighlight() {
    $('form#form #questiontable :input, form#form #questiontable a')
        .bind('click focus select', function(event){
            ...
            var thisDoGreenHighlight = (!isEmbeddedResetLink && !isEmbeddedField);
