        ...
        if ((word.indexOf('[') >= 0) && (!word.match(/\]\[[^\]^\s]+\]\[/)))
        {
            ...
            logicSuggestAjax = $.post(app_path_webroot+'Design/logic_field_suggest.php?pid='+pid, { draft_mode: draft_mode, location: location_plain, word: word.substring(1,word.length)  }, function(data){
                ...
                elem.position({
                    my:        "left top",
                    at:        "left bottom",
                    of:        ($('#rc-ace-editor-parent:visible').length ? $('#rc-ace-editor-parent') : $(location)),
                    collision: "fit"
                });
...
function logicValidate(ob, longitudinal, forceMetadataTable) {
    ...
    var mssg = '<span class="logicValidatorOkay"><img src="'+app_path_images+'tick_small.png">'+langValid+'<span>'+langValidAccuracy+'</span></span>';
    var err_mssg = "<span class='logicValidatorOkay'><img style='position:relative;top:-1px;margin-right:4px;' src='"+app_path_images+"cross_small2.png'>"+langSyntaxError+'<span>'+langValidAccuracy+'</span></span>';
    // timeout to let new text value to enter field;
    // just to back of queue to process
    setTimeout(function() {
...
function specialFunctionsExplanation() {
    // Get content via ajax
    $.get(app_path_webroot+'DataEntry/special_functions_explanation.php',{},function(data){
        var json_data = jQuery.parseJSON(data);
...
function initPage() {
    ...
    $("#menuLnkEditInstr, #menuLnkEditBkmrk, #menuLnkEditReports, .projMenuToggle, #menuLnkProjectFolders, #menuLnkSearchReports, #menuLnkEditDashboards").mouseenter(function() {
...
function openLogicEditor(opener, fullscreen)
{
    ...
    if (fullscreen) {
        ...
        var buttonText1 = langAceEditor1;
    } else {
        var editorStyle = 'height:400px;';
        var dialogWidth = 700;
        var buttonText1 = langAceEditor2;
    }
    // Open dialog
    var html =  '<div id="rc-ace-editor-parent"><div class="mb-3">'+langAceEditorInstr+
                '<span class="nowrap ml-2"><span style="color:#777;margin-right:5px;">'+langLearnHowToUse+'</span><button class="btn btn-xs btn-rcgreen btn-rcgreen-light" style="margin-right:4px;font-size:11px;padding:0px 3px 1px;line-height:14px;" onclick="smartVariableExplainPopup();return false;">[<i class="fas fa-bolt fa-xs" style="margin:0 1px;"></i>] '+langSmartVars+'</button> ' +
                '<button class="btn btn-xs btn-primaryrc btn-primaryrc-light" style="font-size:11px;padding:1px 3px;line-height:14px;"  onclick="specialFunctionsExplanation();return false;"><i class="fas fa-square-root-alt" style="margin:0 2px 0 1px;"></i> '+langSpecialFunc+'</button></span>'+
                '</div><div id="rc-ace-editor" style="'+editorStyle+'"></div><div id="rc-ace-editor-status"></div></div>';
    initDialog('rc-ace-editor-dialog', html);
    var dlgDiv = $('#rc-ace-editor-dialog');
    dlgDiv.dialog({ bgiframe: true, modal: true, width: dialogWidth, title: langAceEditor4, buttons:
        // Cancel
        [{text: langCancel, click: function() {
            try{ dlgDiv.dialog('close'); }catch(e){ }
        }},
        // Update and Close Editor
        { text: langAceEditor3, click: function() {
            // Update val attribute
            opener.attr('val', opener.val());
            //dlgDiv.dialog('option', 'hide', {effect:'blind', duration: 1000 });
    ...
    if (!fullscreen && opener.val().trim() == '') {
        $('#rc-ace-editor').popover({ title: langAceEditor5, content: '<span class="text-secondary fs12">Example: <code>[age] > 30 and [dob] != ""</code></span>', html: true, placement: 'right', delay: 0, trigger: 'manual' }).popover('show');
        setTimeout(function(){
            $('.popover:visible').css({'left':'-228px','top':'-187px', 'z-index':'999'});
        },1);
        setTimeout(function(){
            $('.popover').popover('hide');
        },7000);
