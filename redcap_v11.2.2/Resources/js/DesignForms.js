...
function dialogReevalAutoInvites() {
	...
	$.post(app_path_webroot+'index.php?route=SurveyController:reevalAutoInvites&action=view&pid='+pid, { }, function(data){
		...
		simpleDialog(data,null,'reeval_asi_dlg',700, null, langCancel , 'saveReevalAutoInvites();', asi_024);
	});
}
...
function saveReevalAutoInvites() {
	...
	showProgress(1);
	$.post(app_path_webroot+'index.php?route=SurveyController:reevalAutoInvites&action=save&pid='+pid, { surveysEvents: se.join(',') }, function(data){
		showProgress(0,0);
		if (data == '0') {
			alert(woops);
			return false;
