...
function filter_tags(val) {
	...
	if (val.indexOf('<') > 0 && val.indexOf('>') > 0) {
		...
		var regex = "/(<)([^<>]*)(oncontextmenu\s*=|onkeyup\s*=|onkeydown\s*=|onkeypress\s*=|onhashchange\s*=|onscroll\s*=|onpageshow\s*=|onloadstart\s*=|allowscriptaccess\s*=|onload\s*=|onerror\s*=|onabort\s*=|onclick\s*=|ondblclick\s*=|onblur\s*=|onfocus\s*=|onreset\s*=|onselect\s*=|onsubmit\s*=|onmouseup\s*=|onmouseover\s*=|onmouseout\s*=|onmousemove\s*=|onmousedown\s*=|onmouseenter\s*=|onmouseleave\s*=)([^>]*>)/gi";
		var regex_replace = "$1$2removed=$4";
		var _flag = regex.substr(regex.lastIndexOf(regex[0])+1),
			_pattern = regex.substr(1,regex.lastIndexOf(regex[0])-1),
			regex_raw = new RegExp(_pattern,_flag);
...
function initAutoLogout(resettime,logouttime) {
	...
	$.doTimeout('autoLogoutId3', (logouttime*60000), function(){ $('body').html(''); autoLogoutDialog(warn_timeout3,true,resettime,logouttime); $('.ui-widget-overlay').css({'opacity': '1', 'background-color':'#AAAAAA'}); window.onbeforeunload=function(){ } }, true);
}
...
function enableSldr(fld, ev) {
	...
	$("#slider-" + fld).slider({
		...
		change: function (event, ui) {
			...
			// Piping: Transmit slider value to all piping receiver spans
			setTimeout(function(){
				updatePipeReceivers(fld, event_id, ui.value);
...
function renderSmartChart(id, type, data, labels, orientation, stacked, xlabel, ylabel, hideyvalues, datasetlabels) {
	// Add data
	...
	var ticks = (type == 'bar') ? {beginAtZero:true} : {};
	var scaleLabelX = { display: (xlabel!=''), labelString: xlabel };
	var scaleLabelY = { display: (ylabel!=''), labelString: ylabel };
	var scales = { xAxes: [{ stacked: stacked, ticks: ticks, scaleLabel: scaleLabelX }], yAxes: [{ stacked: stacked, ticks: ticks, scaleLabel: scaleLabelY, display: !hideyvalues }] };
...
var rcDataTable;
function enableFixedTableHdrs(table_id,ordering,searching,searchDom,headerOffset,initCompleteJS) {
...
$(function(){
    ...
    $(document).on('focusin', function(e) {
        if ($(e.target).closest('.tox-tinymce-aux, .moxman-window, .tam-assetmanager-root, .sp-picker-container .sp-input, #rc-ace-editor, .select2-container :input').length) {
