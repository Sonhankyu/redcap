...
class DataEntry
{
	...
	public static function renderRecordHomePage()
	{
			...
			foreach ($Proj->events[$arm]['events'] as $this_event_id=>$this_event) 
			{
				...
				foreach ($this_event_instances as $this_instance) 
				{
					...
					if (!$hasDisplayedEventName) {
					    $evTitle = "<div class='evTitle'>".RCView::escape(strip_tags($this_event['descrip']))."</div>";
		...
		else
		{
			...
			***************************************************************/
			...
			if ($num_records <= DataEntry::$maxNumRecordsHideDropdowns)
			{
                ...
                // If not using auto-numbering, then sort naturally since the record list cache does not perform 100% natural sorting if records contain letters
				if ($order_id_by == "" && !$auto_inc_set) {
		...
		function deleteEventInstance(event_id, instance, isRepeatingEvent) {
			simpleDialog((isRepeatingEvent ? '<?php print js_escape($lang['data_entry_299']) ?>' : '<?php print js_escape($lang['data_entry_240']) ?>'), 
				(isRepeatingEvent ? '<?php print js_escape("{$lang['data_entry_300']} \"".htmlspecialchars((isset($_GET['id']) ? $_GET['id'] : ""), ENT_QUOTES)."\"{$lang['questionmark']}") ?>' : '<?php print js_escape("{$lang['data_entry_238']} \"".htmlspecialchars((isset($_GET['id']) ? $_GET['id'] : ""), ENT_QUOTES)."\"{$lang['questionmark']}") ?>'),
    ...
    public static function renderForm($elements, $element_data=array(), $hideFields=array())
    {
	    ...
	    addLangToJS(array('design_799', 'design_800', 'design_801', 'design_802', 'design_803', 'design_821', 'design_829', 'design_833'));
            ...
            if (!empty($elements_replace))
            {
                ...
                $(function(){
                    <?php if (!empty($piping_transmitter_fields_types['checkbox'])) { ?>
                        // Checkbox fields
                        $("<?php print implode(", ", $piping_transmitter_fields_types['checkbox']) ?>").click(function(event){
                    ...
                    <?php if (!empty($piping_transmitter_fields_types['file'])) { ?>
                        ...
                        updatePipingFileFields("<?php print implode(", ", $piping_transmitter_fields_types['file']) ?>");
                    <?php } ?>
        ...
        if ((PAGE == "DataEntry/index.php") && isset($_GET['id']))
        {
            ...
            print "<div id='formtop-div' class='d-print-none'>";
        ...
        if ($isSurveyPage && !($GLOBALS['public_survey'] && Survey::$nonExistingRecordPublicSurvey) && isset($_GET['event_id']))
        {
            ...
            && isset($Proj->forms[$_GET['page']]['survey_id']) && $hidden_edit == 1 && isset($Proj->forms[$_GET['page']]['survey_id']))
        {
            ...
            if (db_num_rows($q) > 0)
            {
                ...
                if ($repeatInstanceDropdown != "") {
                    print "<div id='formtop-div' class='d-print-none' style='display:block;'>$repeatInstanceDropdown<div class='clear'></div></div>";
                }
                ...
                ?>
                <div class="d-print-none <?php print ($survey_completion_time == "" ? "orange" : "darkgreen") ?>" id="form_response_header">
                    ...
                    if ($survey_completion_time == "") {
                        ...
                        $responseCompletedTimeText = "{$lang['data_entry_101']} {$lang['data_entry_100']} " . DateTimeRC::format_ts_from_ymd($survey_first_submit_time);
                        print RCView::img(array('src'=>'circle_orange_tick.png')) . " <b>$responseCompletedTimeText</b>$participant_identifier" . $lang['period'] . " ";
                    } else {
                        // Complete survey response
                        $responseCompletedTimeText = "{$lang['data_entry_152']} " . DateTimeRC::format_ts_from_ymd($survey_completion_time);
                        print RCView::img(array('src'=>'circle_green_tick.png')) . " <b>$responseCompletedTimeText</b>$participant_identifier" . $lang['period'] . " ";
                    }
                    if ($isPromisInstrument)
                    {
                        if ($isAutoScoringInstrument) {
            ...
            if (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
            {
                ...
                if ($rr_type == "header") {
                    ...
                    $fieldIcons =  '<span class="od-field-icons"><a href="javascript:;" onclick="openAddQuesForm(\'{name}\',\'{rr_type}\',0,\'{signature}\');"
                                    ...
                                    ><img src="'.APP_PATH_IMAGES.'page_copy.png" ignore="Yes" title="Copy" class="REDCapDesignFieldsOption"/></a>&nbsp;
                                    <a href="javascript:;" onClick="moveField(\'{name}\',\'\')"
                                    ><img src="'.APP_PATH_IMAGES.'file_move.png" ignore="Yes" title="Move" class="REDCapDesignFieldsOption"/></a>&nbsp;
                                    ...
                                    ><img src="'.APP_PATH_IMAGES.'cross.png" ignore="Yes" title="Delete Field" class="REDCapDesignFieldsOption"/></a></span>
                                    &nbsp;&nbsp;&nbsp;
...
//                            print "</div>";
                ...
                case 'hidden':
                    ...
                    if (isset($Proj->metadata[$name]) && $Proj->metadata[$name]['element_type'] == 'text' && $name != $Proj->table_pk && !isset($missingDataCodes[$value]))
                    {
                ...
                case 'truefalse':
                    ...
                    print "<div class='resetLinkParent d-print-none'><a href='javascript:;' class='smalllink $randFldClass' tabindex='0' style='display:$reset_radio;'
                        onclick=\"radioResetVal('$name','$formJsName');return false;\">{$lang['form_renderer_20']}</a></div>";
                    DataEntry::displayMissingDataLabel($name, $value);
                    print "$note</td>";
                    break;
                //Yes-No
                ...
                case 'text':
                    ...
                    if (isset($rr_array['validation']))
                    {
                        ...
                        if ($rr_array['validation'] == 'date' || $rr_array['validation'] == 'date_ymd' || $rr_array['validation'] == 'date_mdy' || $rr_array['validation'] == 'date_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ml-2 today-now-btn d-print-none' onclick=\"setToday('$name','{$rr_array['validation']}');return false;\">{$lang['dashboard_32']}</button>";
                        ...
                        elseif ($rr_array['validation'] == 'time')
                        {
                            if (!$disable_all) {
                                $newclass = "time2";
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ml-2 today-now-btn d-print-none' onclick=\"setNowTime('$name');return false;\">{$lang['form_renderer_29']}</button>";
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                $nowBtn .= "<span class='df'>$dformat</span>";
                            }
                        }
                        ...
                        elseif ($rr_array['validation'] == 'datetime' || $rr_array['validation'] == 'datetime_ymd' || $rr_array['validation'] == 'datetime_mdy' || $rr_array['validation'] == 'datetime_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ml-2 today-now-btn d-print-none' onclick=\"setNowDateTime('$name',0,'{$rr_array['validation']}');return false;\">{$lang['form_renderer_29']}</button>";
                                $nowBtn .= "<span class='df'>$dformat</span>";
                            }
                        ...
                        elseif ($rr_array['validation'] == 'datetime_seconds' || $rr_array['validation'] == 'datetime_seconds_ymd' || $rr_array['validation'] == 'datetime_seconds_mdy' || $rr_array['validation'] == 'datetime_seconds_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ml-2 today-now-btn d-print-none' onclick=\"setNowDateTime('$name',1,'{$rr_array['validation']}');return false;\">{$lang['form_renderer_29']}</button>";
                                $nowBtn .= "<span class='df'>$dformat</span>";
                            }
                            // Reformat MDY/DMY values
                    ...
                    if (isset($cp->_fields_utilized[$name]) || ( (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
                        ...
                        ?><script type="text/javascript">$(function(){ $('#<?=$name?>-tr button:not(.rc-autocomplete), #<?=$name?>-tr .ui-datepicker-trigger').hide(); });</script><?php
                        // Add "sv" attribute
                        $sv = "sv='" . (isset($value) ? js_escape($value) : '') . "'";
                        // Add "view equation link"
                        $calcDateParam = (Calculate::isCalcDateField($GLOBALS['status'] < 1 ? $Proj->metadata[$name]['misc'] : $Proj->metadata_temp[$name]['misc']) ? '1' : '0');
                        $calcTextParam = ($calcDateParam == '0' ? '1' : '0');
                        $viewEqLink = "&nbsp;&nbsp;<a href='javascript:;' ignore='Yes' class='viewEq d-print-none' tabindex='-1' onclick=\"viewEq('$name',$calcDateParam,$calcTextParam);\">{$lang['form_renderer_21']}</a>";
                    }
                    ...
                    print  "	<input autocomplete='new-password'{$aria_required} aria-labelledby='$ariaLabelledBy' class='x-form-text x-form-field $newclass' $id type='$rr_type' name='$name' $inputDisabled value='".str_replace("'","&#039;",$value)."'
                                $disabled $id $style $onchange $onclick $onblur $onfocus $tabindex $onkeydown $onkeyup $fv_attr $inputPattern $placeholderText $sv>$viewEqLink
                ...
                case 'calc':
                    ...
                    print  "	<input type='text' aria-labelledby='$ariaLabelledBy' name='$name' value='" . (isset($value) ? $value : '') . "' sv='" . (isset($value) ? $value : '') . "' $id $onfocus tabindex='-1' readonly='readonly'
                                class='x-form-text x-form-field rci-calc'>&nbsp;&nbsp;<a href='javascript:;' ignore='Yes' class='viewEq d-print-none' tabindex='-1' onclick=\"viewEq('$name',0,0);\">{$lang['form_renderer_21']}</a>
                                $note
                            </td>";
                    break;
                ...
                case 'slider':
                    ...
                    $sliderStyle = ($custom_alignment == 'LV') ? "sldrmsgl opacity75" : "sldrmsg opacity75";
                    print  "	<table role='presentation' class='sldrparent $sldrparent_display' style='position:relative;'>
                                    ...
                                    <tr>
                                        <td class='sldrtd' ".($form_locked['status'] ? "" : "onmousedown=\"enableSldr('$name',event.type);$('#slider-$name').attr('modified','1');\"").">
                                            <div id='slider-$name' class='slider' $display data-min='{$rr_array['slider_min']}' data-max='{$rr_array['slider_max']}' data-align='$alignment_class' " . ((isset($value) && $value != '') ? "modified='1'" : '') . " ".($form_locked['status'] ? "locked='1'" : "")."></div>
                                        </td>								
            ...
            foreach ($select_choices_array as $this_value=>$this_text)
            {
                ...
                if ($isMatrixField) {
                    ...
                    if (!$currentChoiceHidden) {
                        print "<input type='radio' id='$rad_id' tabindex='0' $thisChoiceName  $thisChoiceDisabled aria-labelledby='$ariaLabelledBy matrixheader-{$rr_array['grid_name']}-$this_value' $attr value='$this_value' label='".js_escape(strip_tags(label_decode($this_text)))."' ";
                    ...
                    if (!$currentChoiceHidden) {
                        print "<input type='radio' id='$rad_id' tabindex='$inputTabIndex' $thisChoiceName  $thisChoiceDisabled aria-labelledby='$ariaLabelledBy label-$name-$this_value' $attr value='$this_value' ";
                        if ($currentChoiceSelected) print "checked";
                        print ">";
                    }
                    // Enhanced choices
                    if ($enhanced_choices) {
                        ...
                        $enhanced_choices_html .= "<div class='enhancedchoice{$hideChoiceClass} col-12 $col_size_class $thisChoiceDisabledLabel'>"
                            . "<label tabindex='0' for='$rad_id' onkeydown='if(event.keyCode==32){ $enhanced_choices_onclick }' onclick='$enhanced_choices_onclick' comps='$name,value,$this_value' class='$enhanced_choices_class2 $enhanced_choices_class'><span class='ec'>$this_text</span></label></div>";
                    }
                }
                if (!$isMatrixField) print " <label id='label-$name-$this_value' for='$rad_id' class='mc'>$this_text</label>";
                               ...
                               $hideChoices=array(), $ariaLabelledBy="", $maxChoices=array(), $maxChecked=0)
    {
        ...
        if ($select_choices != "")
        {
            ...
            foreach ($select_choices_array as $this_value=>$this_text)
            {
                ...
                if (!$isMatrixField) {
                    print " <label id='label-$name-$this_value' class='mc' for=\"$cid\">$this_text</label>";
                    // Enhanced choices
                    if ($enhanced_choices) {
                        if ($this_text == "") $this_text = "&nbsp;";
                        ...
                        $enhanced_choices_html .= "<div class='enhancedchoice{$hideChoiceClass} col-12 $col_size_class $thisChoiceDisabledLabel'>"
                            . "<label tabindex='0' for='$cid' onkeydown='if(event.keyCode==32){ $enhanced_choices_onclick }' onclick='$enhanced_choices_onclick' comps='$name,code,$this_value' class='$enhanced_choices_class2 $enhanced_choices_class'><span class='ec'>$this_text</span></label></div>";
                    }
                }
                // Finalize output for this choice
                                      ...
                                      $response_id=null, $callSaveRecordHook=false, $surveyJustCompleted=false)
    {
        ...
        if ($saveCalculations && !$Proj->project['disable_autocalcs']) {
            ...
            if (isset($_GET['page']) && isset($Proj->forms[$_GET['page']])) {
                ...
                //$calcFields = is_array($field_values_changed) ? Calculate::getCalcFieldsByTriggerField(array_unique($field_values_changed)) : array();
                // If this is a multi-page survey, only consider this page's fields
                if (PAGE == 'surveys/index.php' && isset($_POST['__page__']) && isset($GLOBALS['pageFields']) && isset($GLOBALS['pageFields'][$_POST['__page__']])) {
                    $calcFields = Calculate::getCalcFieldsByTriggerField(array_merge([$Proj->table_pk], $GLOBALS['pageFields'][$_POST['__page__']]));
                } else {
                    // One-page survey or a data entry form
            ...
            && !empty($Proj->surveys) && isset($_POST[$Proj->project['twilio_delivery_preference_field_map']]))
        {
            ...
            $thisSurveyId = isset($Proj->forms[$_GET['page']]['survey_id']) ? $Proj->forms[$_GET['page']]['survey_id'] : null;
...
//                        order by rl.record_int desc limit 1";
        ...
        ## USE DATA TABLE
            ...
            {
                $sql = "select ifnull(max(cast(substring(record,".(strlen($dag_id)+2).") as signed integer)),0) as thisrecord
                        from redcap_data where project_id = $project_id and field_name = '{$Proj->table_pk}' and record regexp '^({$dag_id}-)([0-9]+)$'";
//                $sql = "select distinct(substring(a.record,".(strlen($dag_id)+2).")) as thisrecord
//                        from redcap_data a
//                        left join redcap_data b on a.project_id = b.project_id and a.record = b.record and b.field_name = '__GROUPID__'
//                        where a.record like '{$dag_id}-%' and a.field_name = '{$Proj->table_pk}'
//                        and a.project_id = $project_id and substring(a.record,".(strlen($dag_id)+2).") regexp '^[0-9]+$'
//                        order by cast(thisrecord as signed integer) desc limit 1";
            }
            // User is not in a DAG
            else
			{
                $sql = "select ifnull(max(cast(record as signed integer)),0) as thisrecord
                        from redcap_data where project_id = $project_id and field_name = '{$Proj->table_pk}' and record regexp '^[0-9]+$'";
//                $sql = "select cast(rl.record_int as char(100)) as thisrecord
//                        from (
//                            select cast(record as signed integer) as record_int
//                            from redcap_data
...
//                        order by rl.record_int desc limit 1";
    ...
    public static function getCalcFields($form)
    {
        ...
        while ($rowcalc = db_fetch_assoc($q))
        {
            ...
            $rowcalc['element_enum'] = Piping::pipeSpecialTags($rowcalc['element_enum'], PROJECT_ID, (isset($_GET['id']) ? $_GET['id'] : ""), (isset($_GET['event_id']) ? $_GET['event_id'] : ""), $_GET['instance'], USERID, true, null, (isset($_GET['page']) ? $_GET['page'] : ""), false, false, false, true);
    ...
    public static function getBranchingFields($form)
    {
        ...
        if (isset($Proj->forms[$form])) {
            ...
            foreach (array_keys($Proj->forms[$form]['fields']) as $this_field)
            {
                ...
                // Pipe any special tags?
                $row['branching_logic'] = Piping::pipeSpecialTags($row['branching_logic'], PROJECT_ID, (isset($_GET['id']) ? $_GET['id'] : ""), (isset($_GET['event_id']) ? $_GET['event_id'] : ""), $_GET['instance'], (defined("USERID") ? USERID : null), true, null, (isset($_GET['page']) ? $_GET['page'] : ""), false, false, false, true);
                //Add this Calc field to Calculate Object for rendering the JavaScript
    ...
    {
        global $Proj, $double_data_entry, $user_rights, $missingDataCodes, $auto_inc_set;
    ...
    public static function initFileUploadPopup()
    {
        ...
        var lang_file_upload_title3 = '<?php echo js_escape($lang['data_entry_459'].' '.$lang['data_entry_468']) ?>';
        </script>
        <?php
