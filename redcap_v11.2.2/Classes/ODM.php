...
class ODM
{
	...
	public static function getProjectAttrMappings($prependRedcapInTag=true, $creatingProject=false, $includeSurveyQueueCustomText=false, $includeDDPFieldMapping=false, $includeDataMartSettings=false)
	{
	...
	public static function getProjectAttrGlobalVars($Proj, $includeSurveyQueueCustomText=false, $includeDDPFieldMapping=false, $includeDataMartSettings=false)
	{
		...
		$attr = "";
		$mappings = self::getProjectAttrMappings(true, false, $includeSurveyQueueCustomText, $includeDDPFieldMapping, $includeDataMartSettings);
	...
	public static function getOdmMetadata($Proj, $outputSurveyFields=false, $outputDags=false, $xml_metadata_options="", $exportAllCustomMetadataOptions=false)
	{
		...
		$xml .= "<GlobalVariables>\n"
			 ...
			 . self::getProjectAttrGlobalVars($Proj, (defined("API") || $exportAllCustomMetadataOptions || in_array('sq', $xml_metadata_options)), (defined("API") || $exportAllCustomMetadataOptions || in_array('ddpmapping', $xml_metadata_options)), (defined("API") || $exportAllCustomMetadataOptions || in_array('datamartsettings', $xml_metadata_options)))
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('dqrules', $xml_metadata_options) ? self::getOdmDataQualityRules() : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('dags', $xml_metadata_options) ? self::getOdmDataAccessGroups() : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('surveys', $xml_metadata_options) ? self::getOdmSurveys() : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('asi', $xml_metadata_options) ? self::getOdmAutomatedSurveyInvitations(in_array('asienable', $xml_metadata_options)) : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('sq', $xml_metadata_options) ? self::getOdmSurveyQueue() : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('userroles', $xml_metadata_options) ? self::getOdmUserRoles() : "")
			 . (defined("API") || $exportAllCustomMetadataOptions || in_array('reports', $xml_metadata_options) ? self::getOdmReports() : "")
		...
		foreach ($itemGroup as $thisGroupOID=>$these_fields) {
			...
			foreach ($these_fields as $this_field) {
				...
				if ($this_field_true !== false) {
					...
					$OntologySearch = ($fieldType == 'text' && isset($Proj->metadata[$this_field]['element_enum']) && strpos($Proj->metadata[$this_field]['element_enum'], ":", 1) !== false) ? " redcap:OntologySearch=\"".RCView::escape($Proj->metadata[$this_field]['element_enum'])."\"" : "";
	...
	public static function parseOdm($xml, $returnCsvDataOnly=false, $removeDagAssignmentsInData=false, $Proj=null)
	{
		...
		$projectAttrMappings = self::getProjectAttrMappings(true, true, true, false, true);
