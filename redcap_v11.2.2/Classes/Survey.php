...
class Survey
{
	...
	{
		global $Proj, $lang, $isAjax, $survey_queue_custom_text, $isMobileDevice, $survey_id, $survey_queue_hide;
	...
	public static function checkConditionsOfRecordToDisplayInQueue($record, $queueItem, $completedSurveyEvents=null, $record_data=null, $event_id=null, $form_name=null, $repeat_instance=1, $repeat_instrument=null)
	{		
	...
	{
		global $lang, $longitudinal, $Proj, $survey_queue_custom_text, $survey_queue_hide;
				...
				$rows .= RCView::tr(array('id'=>"sqtr-$survey_id-$event_id", $queue_item_active_flag=>$queue_item_active_flag_value),
							RCView::td(array('class'=>"data $queue_item_class_firstcell", 'valign'=>'top', 'style'=>'text-align:center;padding:6px;padding-top:10px;'),
								...
								RCView::div(array('id'=>"div_sq_icon_disabled-$survey_id-$event_id", 'style'=>$queue_item_icon_disabled_style),
									...
									RCView::div(array('style'=>'color:#DB2A0F;'), $lang['survey_543']) .
									RCView::div(array('style'=>'padding:20px 0 0;'),
										RCView::button(array('class'=>'jqbuttonsm', 'style'=>'font-size:9px;font-family:tahoma;',
			...
			$html .= "<form id='survey_queue_form'>" .
						...
						RCView::div(array('id'=>'div_survey_queue_custom_text_link', 'style'=>'padding:10px 0 20px 4px;'.$survey_queue_custom_text_add_style),
							RCView::a(array('href'=>'javascript:;', 'style'=>'text-decoration:underline;color:green;', 'onclick'=>"$('#div_survey_queue_custom_text_link').hide();$('#div_survey_queue_custom_text').show('fade'); initMCEEditor('mceSurveyQueueEditor');fitDialog($('#surveyQueueSetupDialog'));"),
								'<i class="fas fa-plus"></i> ' . $lang['survey_541']
							)
						) .
						...
						RCView::div(array('id'=>'div_survey_queue_custom_text', 'class'=>'data', 'style'=>'padding:8px 8px 4px;margin:15px 0 15px;background-color:#eee;'.$survey_queue_custom_text_style),
							...
							RCView::div(array(),
                                ...
                                RCView::div(array('style'=>'display:none;', 'class'=>'mt-1 text-danger fs11', 'id'=>'errMsgContainerIE10'),
                                    $lang['alerts_161']
                                ) .
                        ...
                        RCView::div(array('id'=>'div_survey_queue_hide', 'class'=>'fs14 data mt-1 mb-3 px-3 pt-3 pb-2', 'style'=>'background:#eee;'),
                            ...
                            RCView::label(array('for'=>'survey_queue_hide', 'class'=>'font-weight-bold'), $lang['survey_1296']) .
                            RCView::div(array('class'=>'fs12 pl-4', 'style'=>'line-height:1.1;'),
								$lang['survey_1297']
                            )
                        ) .
				        // Determine if at least one survey in the project has the Survey Auto-Continue feature enabled
                        (!self::anySurveyHasAutoContinueEnabled(PROJECT_ID) ? '' :
							RCView::div(array('class'=>'fs11 text-danger mb-3 mx-2', 'style'=>'line-height:1.2;'),
								'<i class="fas fa-exclamation-circle"></i> '.$lang['survey_1300']
							)
                        ) .
						// Table of surveys
	...
	public static function getFollowupSurveyParticipantIdHash($survey_id, $record, $event_id=null, $forceInsert=false, $instance=null, $projectIdOverride=null)
	{
		...
		if (isset($doInsert) && $doInsert) {
			...
			$sql = "select distinct p.delivery_preference from redcap_surveys_participants p, redcap_surveys_response r
					...
					and r.record = '".db_escape($record)."' and r.instance = $instance and p.survey_id in (".prep_implode(array_keys($Proj->surveys)).") 
	...
	public static function setInvitationPreferenceByParticipantId($participant_id, $delivery_preference)
	{
		...
		// Logging
		if ($success) {
		    Logging::logEvent("$sql1;\n$sql2","redcap_surveys_participants","MANAGE",$participant_id,"participant_id = $participant_id",
	...
	public static function getDeliveryPreferenceFieldMapDataValue($project_id, $record)
	{
		...
		if ($Proj->project['twilio_delivery_preference_field_map'] != '')
		{
			...
			$sql = "select value from redcap_data where field_name = '{$Proj->project['twilio_delivery_preference_field_map']}' and project_id = " . PROJECT_ID . "
                                              ...
                                              $addEmailOption=true, $project_id=null, $returnAllTwilioOptions=false)
	{
		...
		if ($twilio_enabled) {
			...
			if ($returnAllTwilioOptions || $twilio_option_voice_initiate) {
			...
			if ($returnAllTwilioOptions || $twilio_option_sms_invite_make_call) {
			...
			if ($returnAllTwilioOptions || $twilio_option_sms_invite_receive_call) {
	...
	{
		return array(
			'ar-AR_OmarVoice' => 'Arabic (Male)',
			'pt-BR_IsabelaV3Voice' => 'Brazilian Portuguese (Female)',
			'en-GB_CharlotteV3Voice' => 'English - United Kingdom (Female)',
			'en-GB_JamesV3Voice' => 'English - United Kingdom (Male)',
			'en-US_AllisonV3Voice' => 'English - United States (Female)',
			'en-US_HenryV3Voice' => 'English - United States (Male)',
			'fr-FR_NicolasV3Voice' => 'French (Male)',
			'fr-FR_ReneeV3Voice' => 'French (Female)',
			'de-DE_BirgitV3Voice' => 'German (Female)',
			'de-DE_DieterV3Voice' => 'German (Male)',
			'it-IT_FrancescaV3Voice' => 'Italian (Female)',
			'ja-JP_EmiV3Voice' => 'Japanese (Female)',
			'es-ES_EnriqueV3Voice' => 'Spanish - Castilian (Male)',
			'es-ES_LauraV3Voice' => 'Spanish - Castilian (Female)',
			'es-LA_SofiaV3Voice' => 'Spanish - Latin American (Female)',
			'es-US_SofiaV3Voice' => 'Spanish - North American (Female)'
		);
	}
	...
	public static function getThemeCSS($theme_attr)
	{
		...
		if (isset($theme_attr['theme_bg_question'])) {
			$css .= "#econsent_confirm_instr2, #questiontable>tbody>tr>td, #questiontable td.labelmatrix>table>tbody>tr>td, .floatMtxHdr, .floatMtxHdr td { background-image: none; background-color: {$theme_attr['theme_bg_question']}; }\n";
			// Enhanced choices radios/checkboxes
	...
	public static function renderEconsentPdfFrame()
	{
		...
		// Output iframe html
		$backBtnText = $hide_back_button ? "" : "<div id='econsent_confirm_instr2' style='margin:0 10px 15px;text-align:center;font-size:14px;'>{$lang['survey_1170']}</div>";
...
//                    where d.record is null and r.response_id is null";
	...
	public static function decryptResponseHash($hash, $participant_id)
	{
		...
		$sql = "select response_id from redcap_surveys_response where participant_id = $participant_id
				and sha2(concat('$__SALT__',response_id),$algo_num) = '$response_id_hash' limit 1";
		$q = db_query($sql);
		if ($q) {
	...
	{
		global $lang, $pdf_econsent_system_enabled;
        $Proj = new Project($project_id);
		...
		$pdf_content = REDCap::getPDF($record, $instrument, $event_id, false, $instance, $econsentEnabled, $Proj->project['pdf_custom_header_text'], $versionTypeText);
