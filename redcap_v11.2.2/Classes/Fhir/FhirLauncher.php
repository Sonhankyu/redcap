...
class FhirLauncher
{
	...
	{
		$this->startSession();
		if(isset($_SESSION[self::SESSION_NAME]) && $session_data = $_SESSION[self::SESSION_NAME])
		{
	...
	public static function getCookieData()
	{
		...
		return empty($cookie) ? false : unserialize(decrypt($cookie), ['allowed_classes'=>false]);
	}
	...
	public function setSessionData($key, $value)
	{
		...
		if(!isset($_SESSION[self::SESSION_NAME]) || !is_array($_SESSION[self::SESSION_NAME])) $_SESSION[self::SESSION_NAME] = array();
	...
	private function getEhrUserFromUrl()
	{
		...
		if(isset($_GET_lower['user']) && $user = trim(rawurldecode(urldecode($_GET_lower['user']))))
		{
			return $user;
		}
	...
	public function processToken($token)
	{
		...
		$token_ehr_user = isset($token->username) ? trim(rawurldecode(urldecode($token->username))) : null; // try to get the EHR user from the token
