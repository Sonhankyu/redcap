...
class DataMart
{
     ...
     * @return array
     */
    public function getSettings($project_id, $lang=[])
    {   
        $getAppSettings = function() use($project_id, $lang) {
            ...
            return [
                ...
                'lang' => $lang,
                'standalone_authentication_flow' => @$configVals['fhir_standalone_authentication_flow'],
                'standalone_launch_enabled' => @$configVals['fhir_standalone_authentication_flow']==FhirLauncher::MODE_STANDALONE_LAUNCH,
                'standalone_launch_url' => $base_url.FhirLauncher::getStandaloneLaunchUrl(),
                'ehr_system_name' => strip_tags(@$configVals['fhir_source_system_custom_name']),
                'mapping_helper_url' => FhirMappingHelper::getLink($project_id),
            ];
        };
        
        $settings = [
            'fhir_metadata' => $this->getFhirMetadataSource()->getList(),
    ...
    public function runRevision($revision_id, $mrn=null)
    {
        ...
        $addError = function($error) use(&$errors, &$addError) {
            ...
            if($error instanceof \Exception) {
                ...
                if($previous) $message .= PHP_EOL.$previous->getMessage();
                $message .= " (code {$code})";
                $errors[] = new SerializableException($message, $code);
            }
        };
        ...
        try {
            ...
            if($mrn) {
                ...
                $fhirClient = new FhirClient($revision->project_id, $tokenManager);
                $fhirMetadataSource = $this->getFhirMetadataSource();
                ...
                $saveResponse = \Records::saveData($revision->project_id, 'array', $record);
                ...
                if($record_id = key($record)) {
                    ...
                    $fhirStatsCollector = new FhirStatsCollector($revision->project_id, FhirStatsCollector::REDCAP_TOOL_TYPE_CDM);
            ...
            $fhirErrors = $fhirClient ? $fhirClient->getErrors() : [];
            $addError($fhirErrors);
    ...
    {
        $metadataSource = FhirVersionManager::getInstance()->getFhirMetadataSource();
    ...
    {
        $fhirCode = FhirVersionManager::getInstance()->getFhirCode();
        switch ($fhirCode) {
    ...
    public function searchMrns($project_id, $query, $offset=0, $count=10)
    {
        ...
        $query_string = sprintf(
            ...
            $project_id,
            checkNull($formatted_now), checkNull($formatted_now), db_real_escape_string($query)
        );
        if($offset>=0) {
            $query_string .= sprintf(' LIMIT %u, %u', $offset, $count);
        }
        $result = db_query($query_string);
        $total = $getTotal();
        $rows = [];
    ...
    /* public function searchMrnsALTERNATIVE($project_id, $query, $offset=0, $count=10)
    {
        $getTotal = function($main_query) {
            $query_string = sprintf('SELECT COUNT(1) AS `total` FROM ( %s ) AS `main`', $main_query);
            $result = db_query($query_string);
            $row = db_fetch_assoc($result);
            return intval(@$row['total']);
        };
        ...
        $formatted_now = $now->format('Y-m-d H:i');
        $us = chr(31); //unit separator
        $main_query = $query_string = sprintf(
            "SELECT `record`,
            GROUP_CONCAT(CASE WHEN `field_name` = 'mrn' THEN value ELSE NULL END ORDER BY `value` ASC SEPARATOR '{$us}') AS `mrn`,
            GROUP_CONCAT(CASE WHEN `field_name` = 'fetch_date_start' THEN value ELSE NULL END ORDER BY `value` ASC SEPARATOR '{$us}') AS `fetch_date_start`,
            GROUP_CONCAT(CASE WHEN `field_name` = 'fetch_date_end' THEN value ELSE NULL END ORDER BY `value` ASC SEPARATOR '{$us}') AS `fetch_date_end`
