...
class ProjectProxy
{
  ...
  {
    $realtime_webservice_offset_days = @$this->project->project['realtime_webservice_offset_days']; // example "30"
  ...
  public function getFieldValue($record_id, $event_id, $field_name, $instance=false)
  {
      $query_string = sprintf(
          "SELECT `value`, IFNULL(`instance`, 1) as `normalized_instance` FROM redcap_data WHERE
      ...
      while($row = db_fetch_assoc($result)) {
          $instance = @$row['normalized_instance'];
  ...
  /**
   * build the structure of a REDCap recordrecord
   *
   * @param array $carry
   * @param mixed $value
  ...
  public function shouldConvertTimestampFromGmt()
  {
    ...
    $fhir_convert_timestamp_from_gmt = @$this->project->project['fhir_convert_timestamp_from_gmt'];
