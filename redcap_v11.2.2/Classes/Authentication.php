...
class Authentication
{
	...
	public static function authenticate()
	{
		...
		// Oauth2 Azure AD
		elseif ($auth_meth == 'oauth2_azure_ad') {
			// auth via azure ad
		...
		define("ACCESS_SYSTEM_CONFIG", $access_system_config);
		define("ACCESS_SYSTEM_UPGRADE", $access_system_upgrade);
		define("ACCESS_EXTERNAL_MODULE_INSTALL", $access_external_module_install);
		define("ADMIN_RIGHTS", $admin_rights);
		define("ACCESS_ADMIN_DASHBOARDS", $access_admin_dashboards);
		define("ACCESS_CONTROL_CENTER", (SUPER_USER || ACCOUNT_MANAGER || ACCESS_SYSTEM_CONFIG || ACCESS_SYSTEM_UPGRADE || ACCESS_EXTERNAL_MODULE_INSTALL || ADMIN_RIGHTS || ACCESS_ADMIN_DASHBOARDS));
		...
		global $auth_meth, $app_name, $username, $password, $hostname, $db, $institution, $double_data_entry,
			...
			if ($auth_meth == "table" || $auth_meth == "ldap_table" || $auth_meth == "shibboleth_table" || ($auth_meth == "aaf_table" && !isset($_SESSION['tli'])))//***<AAF Modification>***
			{
				...
				if ($isTableBasedUser)
				{
					...
					$pagePromptSetSecurityQuestion = (substr($_SERVER['REQUEST_URI'], strlen($myProjectsUri)*-1) == $myProjectsUri || (defined("PAGE") && PAGE == 'index.php') || (defined("PAGE") && PAGE == 'ProjectSetup/index.php'));
	...
	private static function authenticateOauth2AzureAD()
	{
		...
		global  $auth_meth, $login_logo, $institution, $login_custom_text, $homepage_contact_email, $homepage_contact,
				...
				$rc_autoload_function, $oauth2_azure_ad_primary_admin, $oauth2_azure_ad_secondary_admin;
		...
		else
		{
			...
			$provider = new TheNetworg\OAuth2\Client\Provider\Azure([
				'clientId'          => $oauth2_azure_ad_client_id,
				'clientSecret'      => $oauth2_azure_ad_client_secret,
			...
			// State validation 
				...
				try {
					$userinfo = $provider->get("me", $token);
					...
					if ($rc_user_info['user_email'] == '') {
						...
						$super_admin = 0;
						if ($userid === $oauth2_azure_ad_primary_admin || $userid === $oauth2_azure_ad_secondary_admin) {
							$super_admin = 1;
						}
