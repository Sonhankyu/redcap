...
class DataImport
{
	...
	public static function displayComparisonTable($updateitems, $format='rows', $doODM=false)
	{
		...
		foreach ($updateitems as $studyevent) {
			foreach (array_keys($studyevent) as $fieldname) {
				if (isset($Proj->metadata[$fieldname]) && ($Proj->metadata[$fieldname]['element_type'] == 'calc' || (!$doODM && $Proj->metadata[$fieldname]['element_type'] == 'file')
	...
	public static function renderDataImportToolPage()
	{
		...
		{
			//Print instructions
			print  "<div style='padding-right:10px;line-height:1.4em;max-width:800px;'>";
			...
			else
			{
				print  "	<p style='font-size:14px;font-weight:bold;margin-top:0;'>
                            ...
                            (!$Proj->hasRepeatingFormsEvents() ? "" :
                                "<div style='text-indent:-1.1em;margin-left:2em;margin-top:15px;color:#C00000;'>
			...
			print  "<br><form action='$this_file' method='POST' name='form' enctype='multipart/form-data'>
					<div class='darkgreen' style='max-width:850px;padding:20px;'>
						<div id='uploadmain'>
							...
							<div style='padding-bottom:3px;'>
								<b>{$lang['global_165']}</b> 
								<select name='delimiter' class ='x-form-text x-form-field' style='font-family:tahoma;padding-right:0;padding-top:0;height:22px;margin-left:3px;'> 
							...
							<div style='padding-top:10px;'>
								<button id='submit' name='submit' class='btn btn-xs btn-rcgreen fs14' onclick=\"
									...
									document.form.submit();\">{$lang['data_import_tool_20']}</button>
							</div>
						</div>
						<div id='progress' style='display:none;background-color:#FFF;width:500px;border:1px solid #A5CC7A;color:#800000;'>
							<table cellpadding=10><tr>
