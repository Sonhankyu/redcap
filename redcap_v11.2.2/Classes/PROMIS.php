		...
		foreach ($choicesArray as $attr) {
			if ($enhanced_choices == '1') {
				$choices .= RCView::input(array('class'=>'hidden', 'type'=>'radio', 'value'=>$attr['value'], 'name'=>$attr['name'], 'id'=>$attr['name']))
						 ...
						 .  "<label tabindex='0' onkeydown='if(event.keyCode==32){enhanceChoiceSelect(this,event,null);return false;}' onclick='enhanceChoiceSelect(this,event,null)' for='{$attr['name']}' comps='{$attr['name']},value,{$attr['value']}'><span class='ec'>{$attr['label']}</span></label></div>";
			} else {
