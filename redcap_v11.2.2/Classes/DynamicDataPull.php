...
class DynamicDataPull
{
	...
	public static function getCuratedLoincLabs()
	{
	...
	private static function applyCuratedLoincLabs($metadata_array)
	{
	...
	private static function addLoincCsvToArray($file_path)
	{		
							  ...
							  $project_id=null, $instance=0, $repeat_instrument="")
	{
				...
				foreach ($these_rc_fields as $this_rc_field=>$rc_field_attr) {
					...
					if ($rc_field_attr['temporal_field'] != '') {
						...
						if ($isRepeatingEvent || $Proj->isRepeatingForm($this_event_id, $this_temporal_field_form)) {
							...
							$temporal_field_instances = $temporal_data[$record_identifier_rc]['repeat_instances'][$this_event_id][$this_repeat_form] ?: array();
		...
		// Remove any duplicates in $field_info so that we don't send superfluous data to the data web service		$field_info = array_values(array_map("unserialize", array_unique(array_map("serialize", $field_info))));
	...
	public static function getFhirMetadata()
	{
		...
		$saveCompleteCSV = function($metadata_array) {
			$export_path = APP_PATH_DOCROOT . "Resources/misc/redcap_fhir_metadata_DSTU2.csv";
			$fp = fopen($export_path, 'w');

			$field_names = array_keys(reset($metadata_array));
			fputcsv($fp, $field_names);
			foreach ($metadata_array as $key => $value) {
				fputcsv($fp, $value);
			}
		};
		// $saveCompleteCSV($metadata_array);
	...
	public function getFhirData($mrn, $mapping_list=[])
	{
		...
		try {
			...
			$fhirlogsMapper = new FhirLogsMapper($user_id);
			$recordAdapter = new RecordAdapter();
			$fhirMetadataSource = self::getFhirMetadataSource();
	
			$fhirClient = new FhirClient($project_id, $tokenManager);
			// listen for notifications from the FhirClient
			$fhirClient->attach($fhirlogsMapper, FhirClient::NOTIFICATION_ENTRIES_RECEIVED);
			...
			$fhirClient->attach($tokenManager, FhirClient::NOTIFICATION_REQUEST_ERROR);
			// start the fetching process
			$fhirClient->fetchData($mrn, $mapping_list, $fhirMetadataSource);
		} catch (\Exception $e) {
	 ...
	 */
	public static function getFhirMetadataSource()
	{
		$fhirVersionManager = FhirVersionManager::getInstance();
		$fhirMetadataSource = $fhirVersionManager->getFhirMetadataSource();
		$fhirMetadataSource = new FhirMetadataEmailDecorator($fhirMetadataSource);
		$fhirMetadataSource = new FhirMetadataAdverseEventDecorator($fhirMetadataSource);
		$fhirMetadataSource = new FhirMetadataCdpDecorator($fhirMetadataSource); // do not use encounters and core characteristics in CDP
		return $fhirMetadataSource;
	}
		...
		if ($output_html)
		{
			...
			if(isset($this->fhirData) && $this->fhirData->hasErrors())
			{
				...
				foreach ($errors as $error) {
					...
					if($previous = $error->getPrevious()) $errorMessage .= '<p>'.$previous->getMessage().'</p>';
	...
	{
		$fhirMetadata = self::getFhirMetadataSource();
...
//                            ) .
	 ...
	 */
	private static function checkRecordsModifiedRecently($this_project_id, $records_mrids, $x_days_ago)
	{
	...
	public function getPreviewData($record_identifier_external)
	{
		...
		if ($this->realtime_webservice_type !== 'FHIR') return;
