...
class Logging
{
	...
	{
		// Set conditions
		if (!(defined("UI_ID") && is_numeric(UI_ID) && !defined("API") && !defined("CRON") && defined("PAGE") && PAGE != 'surveys/index.php')) {
			return;
		}
	...
	public static function logPageView($event="PAGE_VIEW", $userid="", $twoFactorLoginMethod=null, $twoFactorForceLoginSuccess=false, $forceTs=null)
	{
		...
		if (!defined('CRON') && defined("PAGE") && !in_array(PAGE, self::$noCountPages)
		...
		else
		{
			switch ($row['event']) {
				...
				case 'MANAGE':
                    ...
                    if (strpos($sql_log, "PDF (with data)") !== false) {
					...
					elseif ($sql_log == "Re-evaluate automated survey invitations") {
						$sql_log = $row['data_values'];
						$id = "<font color='green'>{$lang['asi_040']}</font>";
					}
    ...
    public static function getFilterHTML($getArr, $isApi = false) {
        ...
        foreach ($all_users as $this_user) {
            ...
            $tr_filter_logging .= ">$this_user</option>";
        }
        $tr_filter_logging .= "</select></td></tr>";

        $recordOnChange = ($isApi == false) ?
                            "onchange=\"window.location.href='".PAGE_FULL."?pid=$project_id&logtype='+\$('#logtype').val()+'&dag=&usr='+\$('#usr').val()+'&beginTime='+\$('#beginTime').val()+'&endTime='+\$('#endTime').val()+'&record='+this.value;\""
                            : "";
        $tr_filter_logging .= "<tr>
                                    <td style='text-align:right;padding-right:5px;'>
                                        {$lang['reporting_36']}
		...
		if (Records::getRecordCount($project_id) > DataEntry::$maxNumRecordsHideDropdowns) {
			// Display auto-incomplete text box due to there being so many records
			$tr_filter_logging .= "<input type='text' placeholder='".js_escape($lang['alerts_205'])."' id='record' $disableRecordFilter ";
			if ($isApi == false) $tr_filter_logging .= "class='x-form-text x-form-field'";
			$tr_filter_logging .= " style='margin-bottom:2px;font-size:13px;height:25px;' " . $recordOnChange;
			if (isset($_GET['record'])) {
				$tr_filter_logging .= " value='" . htmlspecialchars($_GET['record'], ENT_QUOTES) . "'";
			}
			$tr_filter_logging .= ">";
		} else {
			$tr_filter_logging .= "<select id='record' $disableRecordFilter ";
			if ($isApi == false) $tr_filter_logging .= "class='x-form-text x-form-field'";
			$tr_filter_logging .= " style='margin-bottom:2px;font-size:13px;height:25px;' " . $recordOnChange . ">
			...
			foreach ($records as $this_record) {
				$tr_filter_logging .= "<option value='" . htmlspecialchars($this_record, ENT_QUOTES) . "' "
					. ((isset($_GET['record']) && $_GET['record'] == $this_record) ? "selected" : "")
					. ">" . strip_tags($this_record) . "</option>";
			}
			unset($records);
			$tr_filter_logging .= "</select>";
		}
        ...
        if ($isApi == false) {
            $tr_filter_logging .= "<div class='btn-group ml-4 bg-white' role='group'>
