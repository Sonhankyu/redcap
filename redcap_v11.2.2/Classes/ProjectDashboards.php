...
class ProjectDashboards
{
	...
	public function renderSetupPage()
	{
		...
		if (isset($_GET['addedit'])) {
			print $this->outputCreateDashboardTable(isset($_GET['dash_id']) ? $_GET['dash_id'] : '');
		} else {
			// JavaScript
			$this->loadDashJS();
	...
	public function viewDash($dash_id)
	{
        ...
        if (!$this->isPublicDash() && defined("USERID") && $dash['user_access'] != 'ALL') {
			...
			// If a user has Setup/Design rights or if they have been given explicit access to this dashboard, allow them to view it
			$noAccess = (!isset($user_list[strtolower(USERID)]) && !$user_rights['design']);
	...
	public function renderDash($dash)
	{
	    ...
	    $pdfBtn = RCView::button(array('class'=>'btn btn-xs btn-defaultrc', 'style'=>'color:#A00000;', 'onclick'=>"exportPageAsPDF('#dashboard_container','".js_escape($exportPdfFileName)."','#dashboard_title_container .btn, #dashboard_title_container a, a.smart-table-export-link, .dash-snapshot, .redcap-chart-colorblind-toggle');"),
                        RCView::fa('fas fa-file-pdf mr-1').$lang['dash_31']
                  );
		$editBtn = '';
		if (!$this->isPublicDash() && isset($user_rights['design']) && $user_rights['design']) {
            $editBtn = RCView::button(array('class' => 'btn btn-xs btn-defaultrc mr-2', 'onclick' => "window.location.href=app_path_webroot+'index.php?pid='+pid+'&route=ProjectDashController:index&addedit=1&dash_id='+getParameterByName('dash_id');"),
                            RCView::fa('fas fa-pencil-alt mr-1') . $lang['global_27']
                       );
		}
		$publicLink = '';
		if (!$this->isPublicDash() && $dash['is_public'] && $GLOBALS['project_dashboard_allow_public'] > 0) {
			...
			$publicLink = RCView::a(array('class' => 'text-primary fs12 mr-3', 'href' => $publicUrl, 'target'=>'_blank'),
				RCView::fa('fas fa-link mr-1') . $lang['dash_52']
			);
        }
				...
				RCView::div(array('id'=>'dashboard_title_container', 'class'=>'clearfix'),
					RCView::div(array('id'=>'dashboard_button_container', 'class'=>'d-print-none', 'style'=>'width:'.$buttonBoxWidth.';'),
						RCView::div(array('class'=>'nowrap dash-snapshot d-print-none mb-2 fs11', 'style'=>'color:#888;'),
                            ...
                            // "Refresh" link
                            (!$showRefreshLink ? "" :
								RCView::a(array('class'=>'fs11 ml-2', 'style'=>'text-decoration:underline;', 'href'=>'javascript:;', 'onclick'=>"resetDashbardCache('{$dash['dash_id']}');"),
	...
	{
		global $lang;
		$tabs = array(	"index.php?route=ProjectDashController:index&addedit=1"=>RCView::fa('fas fa-plus mr-1').$lang['dash_01'],
		...
		if (isset($_GET['dash_id'])) {
		    $tabs["index.php?route=ProjectDashController:index&addedit=1&dash_id=".$_GET['dash_id']] = RCView::fa('fas fa-pencil-alt mr-1').$lang['dash_30'];
        }
	...
	public function getDashboardNames($dash_id=null, $applyUserAccess=false, $fixOrdering=true)
	{
		global $user_rights;

		// Builder SQL to pull dash_id and title in proper order
		if (!$applyUserAccess) {
			$sql = "select distinct r.dash_id, r.title, r.dash_order
		...
		$sql .= is_numeric($dash_id) ? " and r.dash_id = $dash_id" : " order by r.dash_order";
		$q = db_query($sql);
		...
		while ($row = db_fetch_assoc($q)) {
			// Add to array
			$dashboards[] = array('dash_id'=>$row['dash_id'], 'title'=>$row['title']);
			// Check report order
	...
	public function renderDashboardList()
	{
		...
		foreach ($dashboards as $dash_id=>$attr)
		{
			...
			$rows[$item_num][] = RCView::div(array('class'=>'wrap fs14'),
                ...
                // Dashboard name
				RCView::div(array('style'=>'margin-right:'.($attr['is_public'] ? "120px" : "60px").';cursor:pointer;', 'class' => 'dash-title', 'onclick'=>"openDashboard($dash_id);"), RCView::escape($attr['title']))
            );
			// View/export options
			$rows[$item_num][] =
	...
	public function outputCreateDashboardTable($dash_id=null)
	{
		...
		print   RCView::tr(array(),
			...
			RCView::td(array('class'=>'labelrc create_rprt_hdr', 'style'=>'background-color:#e6e6e6;height:50px;padding:5px 10px;'),
				RCView::text(array('name'=>'title', 'value'=>htmlspecialchars($dash['title'], ENT_QUOTES), 'class'=>'x-form-text x-form-field', 'maxlength'=>150, 'onkeydown'=>'if(event.keyCode == 13) return false;', 'style'=>'height:32px;padding: 4px 6px 3px;font-size:16px;width:99%;'))
			)
		);
		...
		if ($publicDashRequiresApproval) {
			$publicDashApprovalText = RCView::div(array('id'=>'public-dash-enable-warning', 'class'=>'font-weight-normal text-danger mt-2 ml-5 fs14'), '<i class="fas fa-info-circle"></i> '.$lang['dash_74']);
			...
			print   RCView::tr(array(),
                        ...
                        RCView::td(array('class' => 'labelrc clearfix px-3 py-2'),
                            RCView::div(array('class' => ''),
								...
								$publicDashApprovalText .
                                // Public link
                                RCView::div(array('id' => 'public_link_div', 'class' => 'mt-1 ml-5' . ($dash['is_public'] ? "" : " hide")),
                                    ($dash['hash'] == ""
                                        ?   // Creating new dash
                                        RCView::span(array('class' => 'fs13 nowrap float-left font-weight-normal text-secondary', 'style' => ''),
                                            $lang['dash_39']
                                        )
                                        :   // Editing existing dash
                                        RCView::span(array('class' => 'fs13 nowrap float-left font-weight-normal mt-1', 'style' => 'margin-right:10px;'),
                                        ...
                                        RCView::button(array('id' => 'create-custom-link-btn', 'class' => 'btn btn-xs btn-defaultrc fs12 ml-4', 'style' => ($dash['short_url'] == '' ? "" : "display:none;"), 'onclick' => "customizeShortUrl('" . js_escape($dash['hash']) . "','$dash_id');return false;"),
                                        ...
                                        RCView::div(array('id' => 'short-link-display', 'class' => 'mt-2', 'style' => 'clear:both;' . ($dash['short_url'] == '' ? "display:none;" : "")),
                                            RCView::span(array('class' => 'fs13 nowrap float-left font-weight-normal mt-1'),
                    ...
                    RCView::td(array('class'=>'labelrc align-top nowrap fs14 p-2', 'style'=>'color:#0450a5;width:160px;'),
						...
						RCView::div(array('class'=>'font-weight-normal', 'style'=>'margin-top:50px;'),
							...
							RCView::button(array('class'=>'btn btn-xs btn-primaryrc fs12', 'onclick'=>"openDashWizard();return false;"),
								'<i class="fas fa-magic mr-1"></i>'.$lang['dash_48']
							)
                        )
                    ) .
                    RCView::td(array('class'=>'labelrc p-2 font-weight-normal'),
						...
						RCView::div(array('class'=>'fs12 mt-3 mb-2 clearfix', 'style'=>'color:#666;'),
							RCView::div(array('class'=>'clearfix mb-2'),
                                RCView::div(array('class'=>'float-left fs13'), $lang['dash_91']) .
                                RCView::div(array('class'=>'float-right mr-3'),
							...
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[aggregate-mean:age]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[aggregate-count:record_id]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[aggregate-max:weight:R-319PCCFN87]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[stats-table:height,weight,bmi,age]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[stats-table:weight,height:min,max,median]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[stats-table:weight,height:user-dag-name]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[scatter-plot:weight,height]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[scatter-plot:weight,height,gender]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[line-chart:weight,height:R-5898NNMYL4]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[line-chart:weight,height,gender]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[pie-chart:education_level]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[pie-chart:race:vanderbilt_dag,duke_dag]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[donut-chart:race:enroll_arm_1,visit1_arm_1]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[bar-chart:ethnicity:R-131EDWCJHN]") .
							RCView::code(array('class'=>'float-left mt-1', 'style'=>'width:272px;'), "[bar-chart:race,gender:bar-vertical,bar-stacked]")
                        )
                    )
                );
        ...
        <div id="dash_wizard_dialog" title="<?php print js_escape2($lang['dash_92']) ?>" class="simpleDialog fs14">
            ...
            <div class="mt-3" style="background-color:#f5f5f5;padding:8px;border:1px solid #ccc;">
                ...
                <!-- Stats table columns -->
                <div class="mt-1 ml-3">
					<?=RCView::div(array('class'=>'d-inline-block', 'style'=>'width:200px;'), $lang['dash_99']) . RCView::select(array('id'=>'smart-thing-table-cols', 'multiple'=>'multiple', 'style'=>'height:70px;', 'class'=>'x-form-text x-form-field fs12 ml-2 mt-3'), $table_col_options, "")?>
                </div>
                <!-- Report filtering -->
                <div class="mt-3">
                    <?=RCView::div(array('class'=>'d-inline-block', 'style'=>'width:200px;'), $lang['dash_98']) . RCView::select(array('id'=>'smart-thing-reports', 'class'=>'x-form-text x-form-field fs12 ml-2'), $report_options, "")?>
                </div>
                <!-- DAG filtering -->
                <?php if (!empty($dags)) { ?>
                <div class="mt-2">
					<?=RCView::div(array('class'=>'d-inline-block', 'style'=>'width:200px;'), $lang['dash_103']) . RCView::select(array('id'=>'smart-thing-dags', 'multiple'=>'multiple', 'style'=>'height:70px;', 'class'=>'x-form-text x-form-field fs12 ml-2 mt-3'), $dag_options, "")?>
                </div>
                <?php } ?>
                <!-- Event filtering -->
				<?php if ($Proj->longitudinal) { ?>
                    <div class="mt-2">
						<?=RCView::div(array('class'=>'d-inline-block', 'style'=>'width:200px;'), $lang['dash_105']) . RCView::select(array('id'=>'smart-thing-events', 'multiple'=>'multiple', 'style'=>'height:70px;', 'class'=>'x-form-text x-form-field fs12 ml-2 mt-3'), $event_options, "")?>
                    </div>
				<?php } ?>
            </div>
            <!-- Display generated smart variable -->
            <div class="mt-4 pt-3 mb-4">
                <div class="font-weight-bold ml-1 fs16" style="color:#A00000;"><?=$lang['dash_93']?></div>
                ...
                <button class="btn btn-primaryrc btn-xs btn-clipboard ml-1" onclick="return false;" title="<?=js_escape2($lang['global_137'])?>" data-clipboard-target="#smart-thing-generated" style="margin-top: 8px;padding:3px 8px 3px 6px;"><i class="fas fa-paste"></i> <?=$lang['global_137']?></button>
            </div>
        </div>
        <?php
	}
	...
	public function outputDashboardPanelTitle()
	{
		...
		$dashList .= "<div style='float:left;margin:2px 0;'>{$lang['global_182']}</div>
		...
		if (isset($user_rights['design']) && $user_rights['design']) {
			$dashList .= "<div class='opacity65' id='menuLnkEditDashboards' style='float:right;margin-right:5px;'>"
				. RCView::i(array('class'=>'fas fa-pencil-alt fs10', 'style'=>'color:#000066;margin-right:2px;'), '')
				. RCView::a(array('href'=>APP_PATH_WEBROOT."index.php?pid=".PROJECT_ID."&route=ProjectDashController:index",'style'=>'font-size:11px;text-decoration:underline;color:#000066;font-weight:normal;'), $lang['dash_28'])
				. "</div>";
		}
		// Return values
	...
	public function outputDashboardPanel()
	{
		global $lang;
		$dashsList = '';
		$dashsMenuList = $this->getDashboardNames(null, !UserRights::isSuperUserNotImpersonator());
		if (!empty($dashsMenuList)) {
			...
			foreach ($dashsMenuList as $attr) {
				...
				$dashsList .= "<div class='hangr'>$num <a href='$dashLink'>".RCView::escape($this_report_name)."</a></div>";
			}
			$dashsList .= "</div>";
		}
		return $dashsList;
