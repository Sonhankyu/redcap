...
class Calculate
{
    ...
    public function feedEquation($name, $string, $field_attr)
	{
		...
		if (!$is_calc_field && self::isCalcDateField($field_attr['misc'])) {
			$string = self::buildDataCalcEquation($field_attr, true, "@CALCDATE($string)"); // Pass last param as override in case piping has resulted in a static value
			if ($string == "") return;
		}
												   ...
												   $current_event_id=null, $group_id=null, $Proj2=null, $bypassFunctionCache=true)
	{
		...
		foreach ($calcFields as $this_field) {
			if (isset($Proj->metadata[$this_field])) {
				...
				if ($isCalcField) {
					...
					$calcFieldsNew[$this_field] = self::buildDataCalcEquation($Proj->metadata[$this_field]);
					$calcFieldsOrder[$this_field] = $Proj->metadata[$this_field]['field_order'];
		...
		if ($Proj->longitudinal) {
			$getDataParams = array('project_id'=>$Proj->project_id, 'records'=>$records, 'field'=>$Proj->table_pk, 'returnEmptyEvents'=>true);
	...
	public static function getCalcFieldsByTriggerField($triggerFields=array(), $do_recursive=true, $Proj2=null)
	{
		...
		foreach ($Proj->metadata as $field=>$attr) {
			...
			elseif ($attr['element_type'] == 'text' && $attr['misc'] != '' && self::isCalcDateField($attr['misc'])
				&& preg_match($regex, Form::getValueInParenthesesActionTag($attr['misc'], "@CALCDATE")))
			{
				$calcFields[] = $field;
			}
			// @CALCTEXT field
			elseif ($attr['element_type'] == 'text' && $attr['misc'] != '' && self::isCalcTextField($attr['misc'])
				&& preg_match($regex, Form::getValueInParenthesesActionTag($attr['misc'], "@CALCTEXT")))
			{
				$calcFields[] = $field;
			}
		}
	...
	{
		return (stripos($misc_action_tags, "@CALCDATE") !== false);
	}

	// Determine if a field is a @CALCTEXT field
	public static function isCalcTextField($misc_action_tags)
	{
		return (stripos($misc_action_tags, "@CALCTEXT") !== false);
	}

	// Take the @CALCDATE text and form it into a calculation
	public static function buildDataCalcEquation($field_attr, $buildForJS=false, $equationOverride=null)
	{
		global $Proj;
		...
		{
			$dataCalcFunction = Form::getValueInParenthesesActionTag(($equationOverride === null ? $field_attr['misc'] : $equationOverride), "@CALCDATE");
			if ($dataCalcFunction != "") {
				// Default
				$dateformat_source = '';
				...
				if (!empty($firstParamFields)) {
					$firstParamField = array_shift($firstParamFields);
					if (isset($Proj->metadata[$firstParamField])) {
						$dateformat_source = substr($Proj->metadata[$firstParamField]['element_validation_type'], -3);
					}
				}
				...
				if ($buildForJS) {
					...
					return "calcdate($dataCalcFunction, '$dateformat_source', '$datatype_return', '$dateformat_return')";
				} else {
					// Build PHP calcdate() function
					return "calcdate($dataCalcFunction, '$datatype_return')";
				}
			}
		}
	...
	public static function buildCalcTextEquation($field_attr, $equationOverride=null)
	{
		...
		if (self::isCalcTextField($field_attr['misc']) && $field_attr['element_type'] == 'text')
		{
			...
			if ($calcTextFunction != "") {
				// Build PHP function
				return "calctext($calcTextFunction)";
			}
		}
		// Return nothing if found nothing
		return "";
