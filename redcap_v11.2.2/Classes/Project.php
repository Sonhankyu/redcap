...
class Project
{
	...
	public static $overwritableGlobalVars = array(	'project_contact_name', 'project_contact_email', 'institution', 'site_org_type',
	...
	public function loadProjectValues()
	{
		...
		{
			if ($key == "auth_meth" && $GLOBALS['auth_meth_global'] != "none" && $value == "none") {
				...
				if (isset($arm_attr['events'][$event_id])) {
					if ($form != null) {
						...
						foreach (array_keys($arm_attr['events']) as $this_event_id) {
							if (isset($this->eventsForms[$this_event_id]) && in_array($form, $this->eventsForms[$this_event_id])) {
	...
	*/
	...
	public function saveEmailInvitationFieldValues($record, $emailFieldsValues=array(), $arm=1, $event_id=null)
	{
		...
		foreach ($emailFieldsValues as $emailField=>$emailValue) 
		{
			...
			$sql = "select distinct '" . $this->project_id . "' as project_id, a.event_id, '" . db_escape($record) . "' as record, 
					'" . db_escape($emailField) . "' as field_name, '" . db_escape($emailValue) . "' as value, a.instance from redcap_data a 
			...
			while ($row = db_fetch_assoc($q)) {
				...
				// If a repeating instrument, then ensure that we have a value for the form status field for that instrument first, else exclude this
				// if ($this->isRepeatingForm($row['event_id'], $this->metadata[$row['field_name']]['form_name'])) {

				// }
				// Add the value
				$sql2 = "insert into redcap_data (project_id, event_id, record, field_name, value, instance) values (".prep_implode($row, true, true).")";
				db_query($sql2);
			}
            ...
            $sql = "select distinct event_id, if(instance is null, 1, instance) as instance, value from redcap_data
					...
					and record = '" . db_escape($record) . "'
					and event_id in (".prep_implode(array_keys($this->events[$arm]['events'])).")";
            if (is_numeric($event_id)) $sql .= " and event_id != ".$event_id; // The current event value has already been logged, so don't do it again.
            $q = db_query($sql);
            while ($row = db_fetch_assoc($q)) {
	...
	public static function setProjectVals()
	{
		...
		// If project has "public/none" authentication, change it manually to system-level auth to prevent there from being any public projects.
		if ($GLOBALS['auth_meth_global'] != "none" && $GLOBALS['auth_meth'] == "none") {
    ...
    public static function getUserDAGRecords()
    {
        ...
        foreach ($projectUsers[PROJECT_ID] as $username => $row)
        {
            ...
            $mappings[] = array(
                'username' => $username,
                'redcap_data_access_group' => !is_null($row['group_id']) ? $Proj->getUniqueGroupNames($row['group_id']) : ''
            );
        }
