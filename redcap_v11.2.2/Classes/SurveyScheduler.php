	...
	*/
	...
	public function getSurveyInvitationLog($record=null, $scheduledInNextXdays=null, $returnCountOnly=false)
	{
		...
		if ((!empty($surveyEmailInvitationFields) || $survey_phone_participant_field != '') && !empty($record_names))
		{
			...
			if ($survey_phone_participant_field != '') {
				$surveyEmailPhoneInvitationFields[''] = $survey_phone_participant_field;
			}
	...
	public function renderSurveyInvitationLog($record=null, $showFullTableDisplay=true, $scheduledInNextXdays=null)
	{
		...
		foreach (array_slice($invitationLog, $limit_begin, $num_per_page) as $row)
		{
			...
			if ($showFullTableDisplay && $row['was_sent'] == '0' && $row['scheduled_status'] != 'DELETED') {
			...
			} elseif ($showFullTableDisplay && $row['scheduled_status'] == 'DELETED') {
			...
			if ($showFullTableDisplay) {
				$rows[$rownum][] = ($row['was_sent'] != '0' || $row['scheduled_status'] == 'DELETED') ? "" : RCView::checkbox(array('id'=>'delssq_'.$row['ssq_id']));
			}
	...
	public function checkConditionsOfRecordToSchedule($survey_id, $event_id, $record)
	{
				...
				|| ($thisSchedule['condition_andor'] == 'AND' && $conditionsPassedSurveyComplete)))
		{
			...
			// Does the logic evaluate as true?
			$conditionsPassedLogic = LogicTester::evaluateLogicSingleRecord($thisSchedule['condition_logic'], $record, null, $this->project_id, 1, null, false, false, true);
		}
		// Check pass/fail values and return boolean if record is ready to have its invitation for this survey/event
		if ($thisSchedule['condition_andor'] == 'OR') {
			// OR
	...
	public function scheduleParticipantInvitation($survey_id, $event_id, $record)
	{
		...
		// Set content
		$row['email_content'] = ($doPipingContent) ? Piping::replaceVariablesInLabel($row['email_content'], $record, $event_id, 1, array(), true, $this->project_id, false, "", 1, false, false, $Proj->surveys[$survey_id]['form_name'], $participant_id, false, false, true) : $row['email_content'];
		## REMINDERS
			...
			while ($row = db_fetch_assoc($q))
			{
				...
				else
				{
					...
					if ($participantEmail != null)
					{
						...
						// Send email
						$invitationSent = $email->send(false, true);
	...
	public function triggerASIs($project_id, $surveysEvents=array())
	{
		$num_scheduled_total = 0;
		$num_deleted_total = 0;
		$num_records_affected_total = 0;
		$recordsAffected = array();
		...
		foreach ($recordList as $key=>$record) {
			...
			list ($numInvitationsScheduled, $numInvitationsDeleted, $numRecordsAffected) = $this->checkToScheduleParticipantInvitation($record, false, $surveysEvents);
			$num_scheduled_total += $numInvitationsScheduled;
			$num_deleted_total += $numInvitationsDeleted;
			// If record was affect, then put it in array
			if (($numInvitationsScheduled + $numInvitationsDeleted) > 0) {
				$recordsAffected[$record] = true;
			}
			// Unset from memory
			unset($recordList[$key]);
		}
		// Return number of records that were scheduled
		return array($num_scheduled_total, $num_deleted_total, count($recordsAffected));
	}
	...
	public function displayAutoInviteSurveyEventCheckboxList($project_id)
	{
		...
		return RCView::p(array('class'=>'mt-0'), $lang['asi_025']) .
						RCView::p(array(), RCView::b($lang['asi_041']).$lang['asi_043']).
						...
						RCView::div(array('class'=>'gray mb-0'),
							...
							RCView::a(array('class'=>'ml-5 fs11', 'onclick'=>"$('#reeval_asi_dlg input[type=checkbox]').prop('checked',true);", 'style'=>'text-decoration:underline'), $lang['survey_41']) .
							RCView::a(array('class'=>'ml-2 fs11', 'onclick'=>"$('#reeval_asi_dlg input[type=checkbox]').prop('checked',false);", 'style'=>'text-decoration:underline'), $lang['survey_42']) .
							$html
						);
	}
