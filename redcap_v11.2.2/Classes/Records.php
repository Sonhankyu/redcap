...
class Records
{
	...
	// Do not perform strict checks if project has more than X records (because the checks take too long and can cause peformance issues)
	...
	public static function deleteForm($project_id, $record, $instrument, $event_id=null, $instance=1, $customLoggingDescription=null)
	{
		...
		if ($checkRecordMultiArms) {
			...
			$sql = "select 1 from redcap_data where project_id = $project_id 
					and record = '".db_escape($record)."' and event_id in (".prep_implode($allArmEvents).") limit 1";
			$q = db_query($sql);
			if (db_num_rows($q) == 0) {
										 ...
										 $arm=null, $limit=null, $limitOffset=0, $filterByRecords=array(), $recursive=false, $recordBeginsWith='', $returnNoOutput=false)
	{
		...
		if ($buildListFromDataTable)
		{
			...
			// Query to get resources from table
			$sql = "select $record_distinct $record_dde_field $event_field from redcap_data where project_id = $project_id
			...
			if (is_numeric($limit)) $sql .= " order by record regexp '^[A-Z]', abs(record), replace(replace(record,'_',''),'-','')*1, record $limitSql";
	...
	public static function getFormStatus($project_id=null, $records=array(), $arm=null, $dag=null, $selected_forms_events_array=array(), $preserve_record_order=false)
	{
		...
		if (!empty($grayStatusRecords)) 
		{
			$sql = "select distinct d.record, d.event_id, m.form_name, d.instance
					...
					and m.element_type != 'calc' and !(m.element_type = 'text' and m.misc is not null and m.misc like '%@CALCTEXT%') and m.field_name != '$table_pk' and d.field_name = m.field_name 
	...
	public static function updateFieldDataValueAllInstances($project_id, $record, $field, $fieldValue, $current_arm=1, $change_reason="")
	{
		...
		if ($fieldValue == '') {
			$sql = "select event_id, instance from redcap_data WHERE project_id = " . $project_id . " AND record = '" . db_escape($record) . "' "
		...
		$sql_all[] = $sql = "DELETE FROM redcap_data WHERE project_id = $project_id AND record = '" . db_escape($record) . "' "
						  . "AND event_id in (" . prep_implode($eventsThisArm) . ") AND field_name = '$field'";
		db_query($sql);
				...
				&& !($write_data_to_file && $hasRepeatedInstances))
			{
				...
				foreach ($sortFields as $this_sort_field) {
					...
					$containsOnlyNumbers = (!$isNumberType && $this_sort_field == $Proj->table_pk && $Proj->project['auto_inc_set']
						...
						&& is_array($record_data) && ctype_digit(str_replace('.', '', implode('', array_keys($record_data)))));
					...
					elseif ($exportAsLabels && isset($mc_choice_labels[$row['field_name']]) && !isset($missingDataCodes[$row['value']])) {
						...
						elseif ($replaceFileUploadDocId && $field_type == 'file' && $row['value'] != '') {
							if (isset($missingDataCodes[$row['value']])) {
								...
								$downloadDocUrl = "<button class='btn btn-defaultrc btn-xs nowrap' style='font-size:8pt;' onclick=\"window.open('$downloadDocBaseUrl&s=&record=$record&event_id=$event_id&instance=$instance&field_name={$row['field_name']}&id={$row['value']}&doc_id_hash=".Files::docIdHash($row['value'])."','_blank');\"><i class=\"fas fa-download fs12\"></i> </button>";
								if ($isRepeatEventOrForm) {
							...
							foreach ($attr3 as $repeat_instrument=>$attr4) {
								foreach ($attr4 as $instance=>$attr5) {
									foreach ($attr5 as $field_name=>$thisEdocId) {
										...
										$docFileName = Files::getEdocName($thisEdocId);
				...
				if ($num_results_returned > $num_per_page)
				{
					...
					if ($num_pages > 1) {
						...
						$pageNumDropdown =  RCView::div(array('class'=>'chklist d-print-none report_pagenum_div clearfix'),
												RCView::div(array('class'=>'float-left', 'style'=>'margin-right:200px;'),
...
//								}
                                        ...
                                        if (isset($missingDataCodes[$this_value]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$this_fieldname]['misc'])) {
											...
											if (!isset($Proj->metadata[$this_fieldname]) && isset($fullCheckboxFields[$this_fieldname])) {
												...
												if (strpos($cell, "http:") !== false || strpos($cell, "https:") !== false) {
													$cell = preg_replace('/(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ims', '<a href="$1" class="text-break fs11 text-decoration-underline" target="_blank">$1</a>', $cell);
												}
												if (strpos($cell, "@") !== false) {
													$cell = preg_replace('/(([a-zA-Z0-9\-\_\.])+@[a-zA-Z\_]+?(\.[a-zA-Z]{2,6})+)/ims', '<a href="mailto:$1" class="text-break fs11 text-decoration-underline" target="_blank">$1</a>', $cell);
												}
											}
                                        }
									}
			...
			elseif ($return_format == 'odm') {
				if ($batchNum < 2) {
					...
					if ($includeOdmMetadata) {
						$xml .= ODM::getOdmMetadata($Proj, $exportSurveyFields, $exportDataAccessGroups, (isset($_GET['xml_metadata_options']) ? $_GET['xml_metadata_options'] : ""), true);
					}
				}
	...
	public static function applyFilteringLogic($logic, $records=array(), $eventsFilter=array(), $project_id=null)
	{
		...
		// Get data
		$record_data = self::getData($Proj->project_id, 'array', $records, array_merge($extra_repeating_instance_fields, $fields_no_events), $eventsGetData);
	...
	public static function addNewAutoIdRecordToCache($project_id, $record, $customRecordName=false)
	{
	...
	public static function saveData()
	{
		...
		try
		{
			...
			// Deal with multiple values for designated email fields
			...
			{
				$nextAutoId = DataEntry::getAutoId($project_id);
			}
			...
			foreach ($records as $key=>&$record) {
				...
				if ($addingAutoNumberedRecords) {
					...
					if (isset($autoNumberedRecords[$oldId])) { 
						...
						if ($commitData) {
							$id = Records::addNewAutoIdRecordToCache($project_id, $nextAutoId++) . "";
						} else {
							$id = $nextAutoId++;
						}
				...
				foreach ($records as $key => &$record)
				{
					...
					foreach ($record as $fieldname => $data)
					{
						...
						if($records[$key][$fieldname]['status'] == 'add' || $records[$key][$fieldname]['status'] == 'update')
						{
							...
							if ($fieldname != $Proj->table_pk && isset($Proj->forms[$fieldForm]['survey_id']) && $Proj->metadata[$fieldname]['element_type'] != 'calc'
								&& isset($responsesCompleted[$this_record_name][$this_event_id][$Proj->forms[$fieldForm]['survey_id']][$repeat_instance_num])
								&& $Proj->surveys[$Proj->forms[$fieldForm]['survey_id']]['pdf_auto_archive'] == '2' && !$bypassEconsentProtection
							) {
								$records[$key][$fieldname]['validation'] = 'error';
							...
							if (isset($metaData[$fieldname]) && $metaData[$fieldname]['element_type'] == 'text' && $metaData[$fieldname]['element_validation_type'] != ''
								...
								&& !Calculate::isCalcTextField($metaData[$fieldname]['misc']))
							{
							...
							if ( isset($metaData[$fieldname]['element_validation_min']) || isset($metaData[$fieldname]['element_validation_max'])  && !isset($missingDataCodes[$newValue]))
							{
								...
								if ($elementValidationMin !== "" && $elementValidationMin !== null)
								{
									...
									if ($metaData[$fieldname]['element_type'] == 'slider' && !isinteger($newValueComp))
									{
										$records[$key][$fieldname]['validation'] = 'error';
									...
									elseif ($newValueComp < $elementValidationMinComp)
									{
										//if hard check
								...
								if ($elementValidationMax !== "" && $elementValidationMax !== null)
								{
									...
									elseif ($newValueComp > $elementValidationMaxComp)
									{
										//if hard check
										if ($metaData[$fieldname]['element_validation_checktype'] == 'hard')
										{
						...
						if ($longitudinal && is_numeric($this_event_id) && $fieldname != 'redcap_event_name' && $fieldname != 'redcap_data_access_group' && $records[$key][$fieldname]['new'] != "" && $fieldname != $Proj->table_pk)
						{
							...
							if (!$bypass_form_event_designation && isset($Proj->metadata[$true_fieldname]) && !in_array($Proj->metadata[$true_fieldname]['form_name'], $Proj->eventsForms[$this_event_id])
			...
			if ($commitData && $group_id == null && $hasDagField)
			{
				...
				foreach ($dagRecordEvent as $record=>$eventdag)
				{
					...
					if ($dataLogging && isset($dagRecordUpdate[$record]) && isset($dag_sql_all) && !empty($dag_sql_all))
					{
	...
	public static function checkRecordNamesCaseSensitive($project_id, $records, $table_pk, $longitudinal)
	{
		...
		$sql = "select distinct record from redcap_data where project_id = $project_id and field_name = '$table_pk'
				and SHA1(record) not in ('" . implode("', '", $records_sha1) . "')
				and record in ('" . implode("', '", array_keys($records_sha1)) . "')";
		$q = db_query($sql);
	...
	public static function addRecordToRecordListCache($project_id, $record, $arm)
	{
		...
		if (!$resetRecordCache) {
			...
			if (!Survey::isPublicSurvey()) {
				$Proj = new Project($project_id);
				if ($Proj->hasGroups()) {
					$sqlg = "(select value from redcap_data where project_id = $project_id and record = '" . db_escape($record) . "' and field_name = '__GROUPID__' limit 1)";
				}
			}
		...
		if (!$resetRecordCache && !Survey::isPublicSurvey() && $recordCount < self::$strictCheckMaxRecordThreshold)
		{
			...
			$rowsInTable = db_result(db_query($sql, $rc_connection), 0);
