...
class System
{
	...
	const minimum_php_version_required = '7.2.5';
	// Set a list of the recommended versions of PHP
	public static $recommendedPhpVersions = array("7.3", "7.4", "8.0");
	...
	public static function init()
	{
		...
		define('ALLOWED_TAGS', '<caption><canvas><ol><ul><li><label><pre><p><a><br><center><font><b><i><u><h6><h5><h4><h3><h2><h1><hr><table><tbody><tr><th><td><thead><tfoot><img><span><div><em><strong><acronym><sub><sup><map><area>');
		// Set error handler
	...
	public static function checkSystemStatus() 
	{
		...
		if ($system_offline && defined("PAGE") && PAGE != 'ControlCenter/check.php' && PAGE != 'ControlCenter/check_server_ping.php' && (!defined('SUPER_USER') || (defined('SUPER_USER') && !SUPER_USER)))
		{
	...
	public static function checkCsrfToken()
	{
		...
		$exemptFromTokenCheck  = ($isExtModPage || $isPlugin || $isApi || (defined("PAGE") && in_array(PAGE, System::$fileDownloadPages)) || (defined("PAGE") && in_array(PAGE, $pagesExemptFromTokenCheck))
								   ...
								   || (defined("PAGE") && (PAGE == "SendItController:upload" || PAGE == "FileRepository/index.php") && empty($_FILES))
	...
	public static function setDbBinLogFormat()
	{
		$q = db_query("select value from redcap_config where field_name = 'db_binlog_format'");
		$db_binlog_format = db_num_rows($q) ? db_result($q, 0) : "";
        if ($db_binlog_format != '') {
			db_query("SET SESSION binlog_format = '" . db_escape($db_binlog_format) . "'");
		}
	}
	...
	public static function isStatementResult($result){
		return (is_object($result) && get_class($result) === 'ExternalModules\StatementResult');
	}
}
