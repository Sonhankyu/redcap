...
class DataQuality
{
	...
	public function __construct()
	{
		...
		$this->predefined_rules = array(
			...
			7 => ($GLOBALS['bypass_branching_erase_field_prompt'] == '1' ? $lang['dataqueries_349'] : $lang['dataqueries_69']).'***',
								...
								foreach ($data2 as $instance=>$data3)
								{
									foreach ($data3 as $field=>$value)
									{
										...
										if ($failed_regex)
										{
											...
											$excludeRecEvt = (isset($excluded[$record][$event_id][$repeat_instrument][$instance][$field]) ? 1 : 0);
											// Save result
											$this->saveLogicCheckResults($rule_id, $record, $event_id, $rule_attr['logic'], $literalLogic, $data_display, $excludeRecEvt, 0, $field, $instance);
											// If record is in a DAG, then get the group_id and increment the DAG discrepancy count
											if (isset($this->dag_records[$record]))
											{
												$group_id = $this->dag_records[$record];
		...
		foreach ($fields as $this_field)
		{
			...
			foreach (array_keys($Proj->eventInfo) as $this_event_id)
			{
				...
				if (isset($Proj->metadata[$this_field]) || in_array($this_field, $specialPipingTags))
				{
					...
					if ($longitudinal && is_array($Proj->eventsForms) && isset($Proj->eventsForms[$this_event_id]) && !in_array($Proj->metadata[$this_field]['form_name'], $Proj->eventsForms[$this_event_id])) continue;
					...
					foreach ($data1 as $repeat_instrument=>$data2)
					{
						foreach ($data2 as $instance=>$instance_data)
						{
							...
							if ($ruleContainsOneField != '' && ($repeat_instrument != '' || $instance != '0')
								&& !$Proj->isRepeatingEvent($event_id) && !$Proj->isRepeatingForm($event_id, $Proj->metadata[$ruleContainsOneField]['form_name'])) continue;
...
//		}
	...
	public function displayRulesTable()
	{
        ...
        $buttons = RCView::div(array('style'=>'text-align:right; font-size:12px;font-weight:normal;margin-bottom:5px;'),
                RCView::button(array('onclick'=>"showBtnDropdownList(this,event,'downloadUploadDQRulesDropdownDiv');", 'class'=>'jqbuttonmed'),
                    RCView::img(array('src'=>'xls.gif', 'style'=>'vertical-align:middle;position:relative;top:-1px;')) .
                    RCView::span(array('style'=>'vertical-align:middle;'), $lang['dataqueries_329']) .
				...
				if ($data_resolution_enabled == '2') {
					...
					if (isset($dataIssues[$results['record']][$results['event_id']])) {
						...
						$dataQueryStatus = ($results['field_name'] == '') ? $dataIssues[$results['record']][$results['event_id']]['query_status']
																		  : (isset($dataIssues[$results['record']][$results['event_id']][$results['field_name']]['query_status']) ? $dataIssues[$results['record']][$results['event_id']][$results['field_name']]['query_status'] : "");
						$dataQueryResponse = ($results['field_name'] == '') ? $dataIssues[$results['record']][$results['event_id']]['response']
																		  : (isset($dataIssues[$results['record']][$results['event_id']][$results['field_name']]['response']) ? $dataIssues[$results['record']][$results['event_id']][$results['field_name']]['response'] : "");
						...
						$this_comment_count = ($results['field_name'] == '') ? $dataIssues[$results['record']][$results['event_id']]['num_comments']
																			 : (isset($dataIssues[$results['record']][$results['event_id']][$results['field_name']]['num_comments']) ? $dataIssues[$results['record']][$results['event_id']][$results['field_name']]['num_comments'] : "");
	...
	public function displayResultsTable($rule_info)
	{
        ...
        $downloadLink = ($num_discrepancies_formatted > 0) ? "<button class='ml-5 btn btn-xs fs12 btn-defaultrc' onclick='javascript: exportRulesDiscrepancies(\"".$rule_id."\");'><img src='".APP_PATH_IMAGES."xls.gif'> {$lang['dataqueries_346']}</button>"
                                                           : "";
		// Set the table title
	...
	static private function renderFieldDataResHistoryExistingSection($attr=array(), $prev_attr=array(), $num_row='1')
	{
		...
		$usersCanRespond = User::getUsersDataResRespond(true, $attr['assigned_user_id']);
		...
		$h = RCView::tr(array('id'=>'res_id-'.(isset($attr['res_id']) ? $attr['res_id'] : "")),
				...
				RCView::td(array('class'=>'data', 'style'=>'border:1px solid #ddd;padding:3px 8px;'.$cellstyle),
					...
					(!$userUploadedFile ? '' :
						RCView::div(array('class'=>'clearfix'),
							...
							RCView::div(array('class'=>'float-right'),
                                RCView::a(array('style'=>'text-decoration:underline;color:#800000;', 'class'=>'fs11 ml-3', 'href'=>'javascript:;', 'onclick'=>"deleteDataQueryFile({$attr['res_id']},{$attr['upload_doc_id']},'".js_escape($lang['dataqueries_326'])."','".js_escape($lang['dataqueries_328'])."','".js_escape($lang['global_53'])."','".js_escape($lang['design_170'])."','".js_escape($lang['design_202'])."');"),
					...
					((!(isset($prev_attr['current_query_status']) && in_array($prev_attr['current_query_status'], array('','VERIFIED','DEVERIFIED')) && $attr['current_query_status'] == 'OPEN')) ? '' :
						RCView::div(array('style'=>''),
					...
					(!($num_row == '1' && $user_rights['data_quality_resolution'] >= 3 && !empty($usersCanRespond) && $attr['current_query_status'] != 'VERIFIED' && !isset($attr['data_values'])) ? '' :
                        RCView::div(array('class'=>'float-right'),
                            RCView::a(array('style'=>'text-decoration:underline;', 'class'=>'fs11 ml-2', 'href'=>'javascript:;', 'onclick'=>"$('#dc-assigned_user_id-div-reassign').show('fade').effect('highlight',2500);"),
                        ...
                        RCView::div(array('id'=>'dc-assigned_user_id-div-reassign', 'style'=>'display:none;border-top:1px dashed #ccc;', 'class'=>'clearfix mt-3 pt-2'),
                            ...
                            RCView::select(array('id'=>'dc-assigned_user_id', 'class'=>'ml-1', 'style'=>'max-width:170px;'), $usersCanRespond, '') .
                            RCView::div(array('class'=>'mt-2 pr-2 fs12 float-left'),
                                $lang['dataqueries_317']
                            ) .
                            RCView::div(array('class'=>'fs12 float-left mt-2'),
							...
							RCView::div(array('class'=>'float-left m-2'),
