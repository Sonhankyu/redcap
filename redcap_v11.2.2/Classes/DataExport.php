...
class DataExport
{
            ...
            <table cellspacing="0" width="100%">
                <tr>
                    <td valign="top" style="padding-left:30px;padding-right:10px;border-right:1px solid #eee;">
                        ...
                        <?php echo $lang['data_export_tool_253']?>
                        <a target="_blank" style="text-decoration:underline;font-weight:normal;" href="https://www.tableau.com/"><?php echo $lang['data_export_tool_254']?></a><?php echo $lang['period']?>
                    ...
                    <td valign="top" style="padding-top:5px;width:120px;text-align:center;">
                        <button class="btn-primaryrc btn btn-xs fs12 wrap" style="width:100px;" onclick="simpleDialog('<?php echo js_escape($tableau_instructions)?>','<?php echo js_escape($lang['data_export_tool_251'])?>', null, '760')"><?=$lang['data_export_tool_284']?></button>
                    </td>
                </tr>
            </table>
	...
	public static function outputCreateReportTable($report_id=null)
	{
        ...
        $publicReportNotModifiable = RCView::div(array('class'=>'blue clearfix mb-3 public-report-no-modify-notice', 'style'=>($report['is_public'] == '1' ? "" : "display:none;")),
			RCView::div(array('class'=>'float-left', 'style'=>'width:45px;'), '<i class="fas fa-info-circle fa-3x"></i>') .
        ...
        if ($GLOBALS['reports_allow_public'] > 0)
        {
			...
			print   RCView::tr(array(),
				...
				RCView::td(array('class' => 'header', 'colspan' => 3, 'style' => 'height:50px;padding:5px 10px;color:#800000;'),
					RCView::div(array('class' => ''),
						...
						// Public link
						RCView::div(array('id' => 'public_link_div', 'class' => 'mt-1 ml-5' . ($report['is_public'] ? "" : " hide")),
							($report['hash'] == ""
								?   // Creating new dash
								RCView::span(array('class' => 'fs13 nowrap float-left font-weight-normal text-secondary', 'id' => 'public_link_div_note_save'),
                                ...
                                ($user_rights['user_rights'] == '0' ? "" :
                                    ...
                                    RCView::button(array('id' => 'create-custom-link-btn', 'class' => 'btn btn-xs btn-defaultrc fs12 ml-4', 'style' => ($report['short_url'] == '' ? "" : "display:none;"), 'onclick' => "customizeShortUrl('" . js_escape($report['hash']) . "','$report_id');return false;"),
		...
		$exportDagSurveyFieldsOptions .= RCView::div(array('style' => 'margin:10px 0 0 20px;text-indent:-18px;'),
			...
			RCView::select(['name'=>'report_display_header', 'class'=>'ml-2 fs12'], ['LABEL'=>$lang['data_comp_tool_26'], 'VARIABLE'=>$lang['data_export_tool_276'], 'BOTH'=>$lang['global_74']], $report['report_display_header'])
		);
		...
		$exportDagSurveyFieldsOptions .= RCView::div(array('style' => 'margin:0 0 0 20px;text-indent:-18px;'),
			...
			RCView::select(['name'=>'report_display_data', 'class'=>'ml-2 fs12'], ['LABEL'=>$lang['data_comp_tool_26'], 'RAW'=>$lang['report_builder_207'], 'BOTH'=>$lang['global_74']], $report['report_display_data'])
		);
		// Render rows
		print   RCView::tr(array(),
					RCView::td(array('class'=>'labelrc create_rprt_hdr', 'colspan'=>4, 'valign'=>'bottom',
        ...
        $instanceLogicTip = "";
        if ($Proj->hasRepeatingFormsEvents()) {
			$instanceLogicTip = RCView::div(array('class'=>'float-right font-weight-normal fs12 mb-1', 'style'=>'max-width:500px;line-height: 1.2;'),
		...
		print   RCView::div(array('class'=>'simpleDialog', 'id'=>'dialog-make-report-public'),
                    ...
                    RCView::div(array('class'=>'hang mb-1', 'style'=>'color:'.($hasRightsToMakePublic ? 'green' : '#A00000')),
                        ($hasRightsToMakePublic
                            ? '<i class="fas fa-check mr-1 d-inline"></i> '.$lang['report_builder_194']
                            : '<i class="fas fa-times mr-2 d-inline"></i> '.$lang['report_builder_195']
                        )
                    ) .
					...
					RCView::div(array('class'=>'hang mb-1', 'style'=>'color:'.(empty($identifierFieldsInReport) ? 'green' : '#A00000')),
						(empty($identifierFieldsInReport)
                            ? '<i class="fas fa-check mr-1 d-inline"></i> '.$lang['report_builder_196']
                            : '<i class="fas fa-times mr-2 d-inline"></i> '.$lang['report_builder_197']." <b>".implode('</b>, <b>', $identifierFieldsInReport)."</b>".$lang['period'].
					...
					RCView::div(array('class'=>'hang mb-1', 'style'=>'color:'.($userViewedReport ? 'green' : '#A00000')),
						($userViewedReport
                            ? '<i class="fas fa-check mr-1 d-inline"></i> '.$lang['report_builder_198']
                            : '<i class="fas fa-times mr-2 d-inline"></i> '.$lang['report_builder_199']." ".
                                RCView::a(array('style'=>'text-decoration:underline;', 'href'=>APP_PATH_WEBROOT."DataExport/index.php?pid={$Proj->project_id}&report_id=$report_id"), $lang['control_center_62']) . " " . $lang['report_builder_200']
                        )
					) .
                    // Checkbox agreements
                    RCView::div(array('class'=>'hang mb-1'),
                        ...
                        RCView::label(array('class'=>'ml-1 d-inline', 'for'=>'make-report-public-checkbox-agreement1'), $lang['report_builder_201'])
                    ) .
					RCView::div(array('class'=>'hang mb-1'),
						...
						RCView::label(array('class'=>'ml-1 d-inline', 'for'=>'make-report-public-checkbox-agreement2'), $lang['report_builder_202'])
					) .
	...
	public static function userHasViewedReportThisSession($report_id, $project_id, $userid)
	{
        ...
        // Query the table
        $sql = "select 1 from redcap_log_view where project_id = $project_id and user = '".db_escape($userid)."' and session_id = '".db_escape(session_id())."'
	...
	{
	    $reportName = self::getReportNames($report_id);
		// Delete report
		...
		// Logging
		Logging::logEvent($sql, "redcap_projects", "MANAGE", $report_id, "report_id = $report_id", "Delete report (report: \"".strip_tags($reportName)."\", report_id: $report_id)");
		// Return success
		return true;
	}
	...
	public static function copyReport($report_id)
	{
		...
		if ($errors == 0) {
			...
			self::checkReportOrder();
			// Logging
			Logging::logEvent($sqlr, "redcap_projects", "MANAGE", $new_report_id, "report_id = $new_report_id\nCopied from report_id = $report_id", "Copy report (report: \"".strip_tags(self::getReportNames($new_report_id))."\", report_id: $new_report_id)");
		}
	...
	public static function renderReportList()
	{
		...
		{
		    if ($report_id == 'SELECTED' && !$user_rights['reports']) continue;
			...
			$rows[$item_num][] = // If the "Selected instruments/events" pre-defined rule, then give other button to open multi-select boxes
								...
								RCView::span(array('class'=>'rprt_btns' . ($report_id == 'SELECTED' ? ' rprt_selected_hidden' : '')),
									...
									// Public link
									(!($report[$report_id]['is_public'] && $user_rights['user_rights'] && $GLOBALS['reports_allow_public'] > 0) ? "" :
										RCView::a(array('href'=>($report[$report_id]['short_url'] == "" ? APP_PATH_WEBROOT_FULL.'surveys/index.php?__report='.$report[$report_id]['hash'] : $report[$report_id]['short_url']), 'target'=>'_blank', 'class'=>'text-primary fs12 nowrap ml-3 ml-1 align-middle'),
											'<i class="fas fa-link"></i> ' .$lang['dash_35']
										)
									)
								) .
                                    ...
                                    $minimizeAmountDataReturned=false, $applyUserDagFilter=true, $bypassReportAccessCheck=false, $excludeMissingDataCodes=false)
	{
		...
		$data_content = Records::getData($Proj->project_id, $returnDataFormat, $records, $fields, $events, $dags, $report['combine_checkbox_values'], $outputDags, $outputSurveyFields,
                                        ...
                                        $report['report_display_header'], $report['report_display_data']);
...
//			$smartParams['filterDags']
//			'records'=>(isset($smartParams['filterDags']) ? $smartParams['filterDags'] : []),
//           'events'=>(isset($smartParams['filterEvents']) ? $smartParams['filterEvents'] : [])
            //print_dump($checkIncludeRecordsEvents);
            //print_array($includeRecordsEvents);
		...
		foreach ($data as $field_name=>$records)
		{
			...
			{
				// Sum
				$fieldStats[$field_name]['sum'] = User::number_format_user(round(array_sum($records), 2), 'auto');
				// Min
				$fieldStats[$field_name]['min'] = User::number_format_user(round(min($records), 2), 'auto');
				// Max
				$fieldStats[$field_name]['max'] = User::number_format_user(round(max($records), 2), 'auto');
				// Mean
				$fieldStats[$field_name]['mean'] = User::number_format_user(round(array_sum($records) / count($records), 2), 'auto');
				// StDev
    ...
    public function publicEnable($report_id, $sendConfirmationEmailToUser=null)
    {
        ...
        if ($GLOBALS['reports_allow_public'] == '1' || ($GLOBALS['reports_allow_public'] == '2' && UserRights::isSuperUserNotImpersonator()))
        {
			// Set in table
    ...
    {
        $sql = "SELECT project_id, report_id, title FROM redcap_reports WHERE hash = '".db_escape($hash)."'";
        $q = db_query($sql);
        if (db_num_rows($q)) {
