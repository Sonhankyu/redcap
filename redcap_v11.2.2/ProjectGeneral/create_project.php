...
if (isset($_POST['copyof']) && is_numeric($_POST['copyof']))
{
	...
	$projectFieldsCopy = array( "repeatforms", "scheduling", "randomization", "surveys_enabled", "field_comment_edit_delete",
								...
								"double_data_entry", "date_shift_max", "shared_library_enabled");
	...
	while ($row = db_fetch_assoc($q))
	{
		$sql = "insert into redcap_surveys (project_id, form_name, title, instructions, acknowledgement, question_by_section,
				...
				offline_instructions, stop_action_acknowledgement, stop_action_delete_response) values
	...
	if ($isTemplate || (isset($_POST['copy_reports']) && $_POST['copy_reports'] == "on"))
	{
		...
		while ($row = db_fetch_assoc($q))
		{
			...
			unset($row['report_id'], $row['unique_report_name'], $row['short_url'], $row['hash'], $row['is_public']);
	...
	Survey::savePidForCustomPublicSurveyStatusChange('survey_pid_create_project', @$_POST['survey_pid_create_project'], $new_project_id);
