...
elseif ($_POST['do_action_status'])
{
	...
	if ($status == 1 && $_POST['archive'] == 0) {
		...
		Survey::savePidForCustomPublicSurveyStatusChange('survey_pid_move_to_analysis_status', @$_POST['survey_pid_move_to_analysis_status'], $project_id);
	// Set to Completed
		...
		Survey::savePidForCustomPublicSurveyStatusChange('survey_pid_mark_completed', @$_POST['survey_pid_mark_completed'], $project_id);
	// Set to Production
	} else {
		$newstatus = 1;
		...
		if ($status == 0) {
			...
			Survey::savePidForCustomPublicSurveyStatusChange('survey_pid_move_to_prod_status', @$_POST['survey_pid_move_to_prod_status'], $project_id);
