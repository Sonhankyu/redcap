...
if (UserRights::isSuperUserNotImpersonator() && $status == '1')
{
	...
	$otherFuncTable .= "<tr>
								<td valign='top'>
									<button class='btn btn-sm btn-primaryrc fs13 nowrap text-left' style='line-height: 1.2;' $moveToDevBtnDisabled onclick='MoveToDev($draft_mode,$usingDTS)'>
...
if ($display_project_xml_backup_option || $allow_create_db)
{
	...
	if ($display_project_xml_backup_option)
	{
		...
		if (!empty($projectDashboards)) $xmlOptions .= "<div class='ml-2 fs11'><input class='xml_options' style='position:relative;top:2px;' type='checkbox' value='projectdashboards' checked> {$lang['global_182']}</div>";
		$alert = new Alerts();
		...
		// Data Mart: settings and revisions checkbox
		if(DataMart::isEnabled(PROJECT_ID)) $xmlOptions .= "<div class='ml-2 fs11'><input class='xml_options' style='position:relative;top:2px;' type='checkbox' value='datamartsettings' checked> {$lang['data_export_tool_246']}</div>";
